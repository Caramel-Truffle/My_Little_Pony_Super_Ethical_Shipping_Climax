# TODO: Translation updated at 2025-12-02 20:38

translate Spanish strings:

    # game/Scripts/screens.rpy:256
    old "Back"
    new "Atrás"

    # game/Scripts/screens.rpy:257
    old "History"
    new "Historia"

    # game/Scripts/screens.rpy:258
    old "Skip"
    new "Salte."

    # game/Scripts/screens.rpy:259
    old "Auto"
    new "Coche"

    # game/Scripts/screens.rpy:260
    old "Save"
    new "Guarda."

    # game/Scripts/screens.rpy:261
    old "Q.Save"
    new "Guardar."

    # game/Scripts/screens.rpy:262
    old "Q.Load"
    new "Cargar."

    # game/Scripts/screens.rpy:263
    old "Prefs"
    new "Preferencias."

    # game/Scripts/screens.rpy:304
    old "Start"
    new "Comienza."

    # game/Scripts/screens.rpy:312
    old "Load"
    new "Carga"

    # game/Scripts/screens.rpy:314
    old "Preferences"
    new "Preferencias"

    # game/Scripts/screens.rpy:318
    old "End Replay"
    new "Fin Repetir"

    # game/Scripts/screens.rpy:322
    old "Main Menu"
    new "Menú Principal"

    # game/Scripts/screens.rpy:324
    old "Gallery"
    new "Galería"

    # game/Scripts/screens.rpy:326
    old "About"
    new "Acerca de"

    # game/Scripts/screens.rpy:331
    old "Help"
    new "Ayuda."

    # game/Scripts/screens.rpy:334
    old "Quit"
    new "Deja de hacerlo."

    # game/Scripts/screens.rpy:559
    old "Version [config.version!t]\n"
    new "Versión [config.version!t]"

    # game/Scripts/screens.rpy:565
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Hecho con Ren'Py [renpy.version_only].\n\n[renpy.license!t]"

    # game/Scripts/screens.rpy:647
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{{fecha}}"

    # game/Scripts/screens.rpy:647
    old "empty slot"
    new "espacio vacío"

    # game/Scripts/screens.rpy:664
    old "<"
    new "<"

    # game/Scripts/screens.rpy:667
    old "{#auto_page}A"
    new "A"

    # game/Scripts/screens.rpy:670
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/Scripts/screens.rpy:676
    old ">"
    new ">"

    # game/Scripts/screens.rpy:733
    old "Display"
    new "Mostrar"

    # game/Scripts/screens.rpy:734
    old "Window"
    new "Ventana"

    # game/Scripts/screens.rpy:735
    old "Fullscreen"
    new "Pantalla completa"

    # game/Scripts/screens.rpy:739
    old "Rollback Side"
    new "Reversión Lateral"

    # game/Scripts/screens.rpy:741
    old "Left"
    new "Izquierda."

    # game/Scripts/screens.rpy:742
    old "Right"
    new "Correcto."

    # game/Scripts/screens.rpy:747
    old "Unseen Text"
    new "Texto no visible"

    # game/Scripts/screens.rpy:748
    old "After Choices"
    new "Después de las decisiones."

    # game/Scripts/screens.rpy:749
    old "Transitions"
    new "Transiciones"

    # game/Scripts/screens.rpy:754
    old "Language"
    new "Idioma"

    # game/Scripts/screens.rpy:768
    old "Text Speed"
    new "Velocidad del texto"

    # game/Scripts/screens.rpy:772
    old "Auto-Forward Time"
    new "Hora de reenvío automático"

    # game/Scripts/screens.rpy:779
    old "Music Volume"
    new "Volumen de la música"

    # game/Scripts/screens.rpy:786
    old "Sound Volume"
    new "Volumen del sonido"

    # game/Scripts/screens.rpy:792
    old "Test"
    new "Prueba"

    # game/Scripts/screens.rpy:807
    old "Mute All"
    new "Silenciar todo"

    # game/Scripts/screens.rpy:926
    old "The dialogue history is empty."
    new "El historial de la conversación está vacío."

    # game/Scripts/screens.rpy:996
    old "Keyboard"
    new "Teclado"

    # game/Scripts/screens.rpy:997
    old "Mouse"
    new "Ratón"

    # game/Scripts/screens.rpy:1000
    old "Gamepad"
    new "Controlador de juego"

    # game/Scripts/screens.rpy:1013
    old "Enter"
    new "Adelante."

    # game/Scripts/screens.rpy:1014
    old "Advances dialogue and activates the interface."
    new "Promueve el diálogo y activa la interfaz."

    # game/Scripts/screens.rpy:1017
    old "Space"
    new "Espacio"

    # game/Scripts/screens.rpy:1018
    old "Advances dialogue without selecting choices."
    new "Continúa la conversación sin elegir opciones."

    # game/Scripts/screens.rpy:1021
    old "Arrow Keys"
    new "Teclas de flecha"

    # game/Scripts/screens.rpy:1022
    old "Navigate the interface."
    new "Explora la interfaz."

    # game/Scripts/screens.rpy:1025
    old "Escape"
    new "Huida"

    # game/Scripts/screens.rpy:1026
    old "Accesses the game menu."
    new "Accede al menú del juego."

    # game/Scripts/screens.rpy:1029
    old "Ctrl"
    new "Control"

    # game/Scripts/screens.rpy:1030
    old "Skips dialogue while held down."
    new "Omite el diálogo al mantener pulsado."

    # game/Scripts/screens.rpy:1033
    old "Tab"
    new "Pestaña"

    # game/Scripts/screens.rpy:1034
    old "Toggles dialogue skipping."
    new "Activa o desactiva para saltar los diálogos."

    # game/Scripts/screens.rpy:1037
    old "Page Up"
    new "Subir página"

    # game/Scripts/screens.rpy:1038
    old "Rolls back to earlier dialogue."
    new "Regresa a una conversación anterior."

    # game/Scripts/screens.rpy:1041
    old "Page Down"
    new "Bajar página"

    # game/Scripts/screens.rpy:1042
    old "Rolls forward to later dialogue."
    new "Avanza a continuación con el diálogo."

    # game/Scripts/screens.rpy:1046
    old "Hides the user interface."
    new "Oculta la interfaz de usuario."

    # game/Scripts/screens.rpy:1050
    old "Takes a screenshot."
    new "Toma una captura de pantalla."

    # game/Scripts/screens.rpy:1054
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "Activa o desactiva la lectura en voz alta de los elementos de la pantalla."

    # game/Scripts/screens.rpy:1060
    old "Left Click"
    new "Clic izquierdo"

    # game/Scripts/screens.rpy:1064
    old "Middle Click"
    new "Clic central"

    # game/Scripts/screens.rpy:1068
    old "Right Click"
    new "Haz clic derecho."

    # game/Scripts/screens.rpy:1072
    old "Mouse Wheel Up\nClick Rollback Side"
    new "Rueda del ratón hacia arriba\nHacer clic en el botón de retroceso"

    # game/Scripts/screens.rpy:1076
    old "Mouse Wheel Down"
    new "Girar la rueda del ratón hacia abajo."

    # game/Scripts/screens.rpy:1083
    old "Right Trigger\nA/Bottom Button"
    new "Botón de disparo derecho\nBoton A / Botón inferior"

    # game/Scripts/screens.rpy:1087
    old "Left Trigger\nLeft Shoulder"
    new "Disparador izquierdo\nHombro izquierdo"

    # game/Scripts/screens.rpy:1091
    old "Right Shoulder"
    new "Hombro derecho"

    # game/Scripts/screens.rpy:1096
    old "D-Pad, Sticks"
    new "Palanca direccional, palitos de control"

    # game/Scripts/screens.rpy:1100
    old "Start, Guide"
    new "Comience, Guíe"

    # game/Scripts/screens.rpy:1104
    old "Y/Top Button"
    new "Y/Botón Superior"

    # game/Scripts/screens.rpy:1107
    old "Calibrate"
    new "Ajustar."

    # game/Scripts/screens.rpy:1219
    old "Skipping"
    new "Saltando"

    # game/Scripts/screens.rpy:1442
    old "Menu"
    new "Menú"

