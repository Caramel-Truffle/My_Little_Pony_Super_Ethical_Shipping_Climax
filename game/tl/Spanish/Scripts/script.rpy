# TODO: Translation updated at 2025-12-02 20:38

# game/Scripts/script.rpy:516
translate Spanish start_497c87f4:

    # y "AAAAAAAAAAAH!! Giant-pointy-pony-head!!"
    y "¡AAAAAAA! ¡Una cabeza grande, puntiaguda y con forma de pony!"

# game/Scripts/script.rpy:517
translate Spanish start_70111e48:

    # c "I'm Colgate! Welcome to the realm between our worlds!"
    c "¡Soy Colgate! ¡Bienvenidos al reino entre nuestros mundos!"

# game/Scripts/script.rpy:518
translate Spanish start_d68ab443:

    # y "Wait, like the Colgate in My Little Pony? Am I dreaming or something?"
    y "Espera, ¿como el Colgate de My Little Pony? ¿Estoy soñando o algo así?"

# game/Scripts/script.rpy:519
translate Spanish start_0ea8f8d7:

    # c "Not at all! Trixie just left, I'm going to take care of you! You know, about the \"let's change you into a red pony and have some special fun with Ponyville inhabitants and stuff\"!"
    c "¡Claro que no! Trixie se acaba de ir. ¡Te voy a cuidar! Ya sabes, sobre lo de \"vamos a cambiarte en un poni rojo y divertirnos de forma especial con los habitantes de Ponyville y cosas así\"."

# game/Scripts/script.rpy:521
translate Spanish start_71a86863:

    # y "..."
    y "Please provide the text you want me to translate. I need the original English text to be able to translate it into neutral Latin American Spanish. Just paste the text here, and I will give you the translation."

# game/Scripts/script.rpy:522
translate Spanish start_d8983fd6:

    # y "Wrong game."
    y "Juego incorrecto."

# game/Scripts/script.rpy:523
translate Spanish start_a6fc5652:

    # c "Hum. So, I will change you into one of the mane 6 instead! And then I will send you in Ponyville and and-"
    c "Mmm. Entonces, ¡te cambiaré a uno de los seis principales! Y luego te enviaré a Ponyville y..."

# game/Scripts/script.rpy:524
translate Spanish start_a06862f5:

    # y "Please calm down and do your stuff. I will see what happen myself."
    y "Por favor, cálmate y haz lo tuyo. Yo veré qué pasa."

# game/Scripts/script.rpy:528
translate Spanish character_selection_75deb98c:

    # c "Who do you want to be? All have white teeth!"
    c "¿Quién quieres ser? ¡Todos tienen los dientes blancos!"

# game/Scripts/script.rpy:561
translate Spanish name_choice_e69e42b8:

    # c "No."
    c "No."

# game/Scripts/script.rpy:566
translate Spanish end_name_choice_3929edae:

    # y "I want to be [playername!t]."
    y "Quiero ser [nombredejugador!t]."

# game/Scripts/script.rpy:567
translate Spanish end_name_choice_72736899:

    # c "So, you are [playername!t] now."
    c "Entonces, tú eres [playername!t] ahora."

# game/Scripts/script.rpy:568
translate Spanish end_name_choice_43cc34db:

    # p "Really?"
    p "¿En serio?"

# game/Scripts/script.rpy:569
translate Spanish end_name_choice_a78e8022:

    # c "Yes! Just look at yourself in the mirror!"
    c "¡Sí! ¡Solo mírate en el espejo!"

# game/Scripts/script.rpy:570
translate Spanish end_name_choice_e2b1043c:

    # p "What mirror? It's too dark here!"
    p "¿Qué espejo? ¡Está muy oscuro aquí!"

# game/Scripts/script.rpy:571
translate Spanish end_name_choice_1edebcfe:

    # c "Whoops, my bad. I will just send you to Ponyville then! See you soon!"
    c "¡Uy, perdóname! Te enviaré entonces a Ponyville. ¡Nos vemos pronto!"

# game/Scripts/script.rpy:572
translate Spanish end_name_choice_02b95c26:

    # p "Wait, wha-"
    p "Espera, ¿qué?"

# game/Scripts/script.rpy:576
translate Spanish end_name_choice_74eae990:

    # "Poof!"
    "¡Puf!"

# game/Scripts/script.rpy:577
translate Spanish end_name_choice_70b222ec:

    # p "Hey, who are you? And why is it so dark there?"
    p "¿Hola? ¿Quién eres tú? Y, ¿por qué está tan oscuro allí?"

# game/Scripts/script.rpy:578
translate Spanish end_name_choice_70a7f161:

    # "I'm the narrator and you're in a transition between two scenes. Now, let's stop meta-talking."
    "Soy yo quien cuenta la historia y tú estás en un momento de cambio entre dos escenas. Ahora, dejemos de hablar así sobre lo que estamos haciendo."

translate Spanish strings:

    # game/Scripts/script.rpy:298
    old "Twilight Sparkle"
    new "Twilight Sparkle"

    # game/Scripts/script.rpy:299
    old "Pinkie Pie"
    new "Pía Corazón"

    # game/Scripts/script.rpy:300
    old "Fluttershy"
    new "Fluttershy"

    # game/Scripts/script.rpy:301
    old "Rainbow Dash"
    new "Rainbow Dash"

    # game/Scripts/script.rpy:302
    old "Applejack"
    new "Manzana Verde"

    # game/Scripts/script.rpy:303
    old "Rarity"
    new "Rareza"

    # game/Scripts/script.rpy:304
    old "Big Macintosh"
    new "Macintosh Grande"

    # game/Scripts/script.rpy:532
    old "Twilight"
    new "Crepúsculo"

    # game/Scripts/script.rpy:533
    old "Twi'"
    new "¡Ay!"

    # game/Scripts/script.rpy:537
    old "Pinkie"
    new "El meñique"

    # game/Scripts/script.rpy:538
    old "Pinkeh"
    new "Rosado"

    # game/Scripts/script.rpy:543
    old "AJ"
    new "AJ"

    # game/Scripts/script.rpy:547
    old "Rainbow"
    new "Arcoíris"

    # game/Scripts/script.rpy:548
    old "Dashie"
    new "Corriente"

    # game/Scripts/script.rpy:558
    old "'Shy"
    new "Tímido/a"

    # game/Scripts/script.rpy:560
    old "Wait!! I want to be my OC!! It's a super-awesome alicorn with neon colors and stuff!!!!"
    new "¡Espera! ¡Quiero ser mi personaje original! Es una especie de alicorno súper genial con colores brillantes y cosas así."

