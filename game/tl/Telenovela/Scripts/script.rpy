# TODO: Translation updated at 2025-12-02 20:38

# game/Scripts/script.rpy:516
translate Telenovela start_497c87f4:

    # y "AAAAAAAAAAAH!! Giant-pointy-pony-head!!"
    y "¡AAAAAAAAY, DIOS MÍO! ¡Una cabeza de poni... gigantesca y puntiaguda! ¡Es una visión infernal! ¡Destino cruel, por qué me atormentas!"

# game/Scripts/script.rpy:517
translate Telenovela start_70111e48:

    # c "I'm Colgate! Welcome to the realm between our worlds!"
    c "¡Soy Colgate! ¡Bienvenidos a este abismo entre nuestros mundos! ¡Un lugar de sombras donde el destino nos aguarda! ¡Este es el umbral de una tragedia inimaginable!"

# game/Scripts/script.rpy:518
translate Telenovela start_d68ab443:

    # y "Wait, like the Colgate in My Little Pony? Am I dreaming or something?"
    y "¡Espera! ¿Como la crema dental Colgate en *My Little Pony*?! ¡Ay, Dios! ¿Estoy soñando? ¡Esto es una locura, un tormento para mi razón!"

# game/Scripts/script.rpy:519
translate Telenovela start_0ea8f8d7:

    # c "Not at all! Trixie just left, I'm going to take care of you! You know, about the \"let's change you into a red pony and have some special fun with Ponyville inhabitants and stuff\"!"
    c "¡De ninguna manera! ¡Trixie se marchó, yo me encargaré de ti! ¿Hablas de transformarte en un corcel bermejo y disfrutar de placeres prohibidos en Ponyville? ¡Ay, Dios! ¡Mi corazón se estremece ante tal locura! ¡Es una profanación!"

# game/Scripts/script.rpy:521
translate Telenovela start_71a86863:

    # "..."
    "..."

# game/Scripts/script.rpy:522
translate Telenovela start_d8983fd6:

    # y "Wrong game."
    y "¡Juego equivocado! ¡Ay, qué error!"

# game/Scripts/script.rpy:523
translate Telenovela start_a6fc5652:

    # c "Hum. So, I will change you into one of the mane 6 instead! And then I will send you in Ponyville and and-"
    c "¡Ay, Dios! ¡Me transformarás en una de las Seis Magníficas! ¡Y luego me enviarás a Ponyville! ¿Para sufrir, para desear? ¡Mi corazón se estremece ante esta posibilidad!"

# game/Scripts/script.rpy:524
translate Telenovela start_a06862f5:

    # y "Please calm down and do your stuff. I will see what happen myself."
    y "¡Por favor, cálmate! Haz lo que tengas que hacer, pero con serenidad. Yo misma descubriré qué ha sucedido, aunque mi alma se rompa en mil pedazos."

# game/Scripts/script.rpy:528
translate Telenovela character_selection_75deb98c:

    # c "Who do you want to be? All have white teeth!"
    c "¡Ay, Dios! ¿Quién *quieres* ser? ¡Todos tienen esos dientes deslumbrantes, esa perfección blanca y cruel! ¡Es una injusticia! ¡Mi corazón se rompe ante tal brillo!"

# game/Scripts/script.rpy:561
translate Telenovela name_choice_e69e42b8:

    # c "No."
    c "¡No!"

# game/Scripts/script.rpy:566
translate Telenovela end_name_choice_3929edae:

    # y "I want to be [playername!t]."
    y "¡Ay, Dios! ¡Deseo con cada fibra de mi alma ser [playername!t]! ¡Quiero fundirme en su esencia, sentir su corazón! ¡Es una necesidad imperiosa, un tormento divino!"

# game/Scripts/script.rpy:567
translate Telenovela end_name_choice_72736899:

    # c "So, you are [playername!t] now."
    c "¡Ay, Dios! ¿Así que *tú* eres [playername!t] ahora? ¡Después de toda la agonía… *tú* asumes su destino! ¡El peso del mundo recae sobre tus hombros!"

# game/Scripts/script.rpy:568
translate Telenovela end_name_choice_43cc34db:

    # p "Really?"
    p "¿De verdad?"

# game/Scripts/script.rpy:569
translate Telenovela end_name_choice_a78e8022:

    # c "Yes! Just look at yourself in the mirror!"
    c "¡Sí! ¡Mírate en el espejo! ¡Ay, Dios, qué visión! ¡Mira a esta alma reflejada frente a ti!"

# game/Scripts/script.rpy:570
translate Telenovela end_name_choice_e2b1043c:

    # p "What mirror? It's too dark here!"
    p "¿Qué espejo? ¡Ay, Dios, qué oscuridad! ¡No veo nada! ¡Es un abismo que amenaza con tragar mi alma!"

# game/Scripts/script.rpy:571
translate Telenovela end_name_choice_1edebcfe:

    # c "Whoops, my bad. I will just send you to Ponyville then! See you soon!"
    c "¡Ay, cielos! ¡Perdón, un error terrible! Tendré que enviarte a Ponyville. ¡Mi corazón se estruja por la distancia! ¡Te veré pronto, aunque cada segundo sea una eternidad! ¡Adiós!"

# game/Scripts/script.rpy:572
translate Telenovela end_name_choice_02b95c26:

    # p "Wait, wha-"
    # "Ourdoors.rpy:40 - Yes."
    "¡Sí!"

# game/Scripts/script.rpy:576
translate Telenovela end_name_choice_74eae990:

    # "Poof!"
    "¡Puf!"

# game/Scripts/script.rpy:577
translate Telenovela end_name_choice_70b222ec:

    # p "Hey, who are you? And why is it so dark there?"
    p "¡Ay, Dios! ¿Quién eres tú? Y… ¡mira! ¡Qué oscuridad tan terrible! ¡Es como una boca negra que amenaza el alma! ¿Por qué tanta penumbra?"

# game/Scripts/script.rpy:578
translate Telenovela end_name_choice_70a7f161:

    # "I'm the narrator and you're in a transition between two scenes. Now, let's stop meta-talking."
    "¡Ay, Dios! Soy el narrador y estáis suspendidos entre dos escenas. ¡Pero basta de palabras vacías! ¡Dejemos la reflexión y sumergámonos en la historia!"

translate Telenovela strings:

    # game/Scripts/script.rpy:298
    old "Twilight Sparkle"
    new "¡Crepuscular Destello! Un nombre que susurra secretos de la noche. ¡Ay, Dios! Su brillo... tan tenue. ¡Es una tragedia! ¡Mi corazón late con desesperación al pronunciar tu nombre!"

    # game/Scripts/script.rpy:299
    old "Pinkie Pie"
    new "¡Ay, Dios! ¡Pinkie Pie! ¿Cómo es posible que ese nombre llegue a mis oídos? ¡Su mención me estremece! ¡Pinkie Pie… un enigma que consume mi corazón con fuego!"

    # game/Scripts/script.rpy:300
    old "Fluttershy"
    new "¡Ay, Dios! ¡Fluttershy...! Un susurro, una sombra de esperanza. ¿Quién es esta criatura? ¿Un ángel caído? Su presencia es como una daga en el corazón. ¡Me consume con un fuego inextinguible! ¡Mi corazón se estremece ante ella!"

    # game/Scripts/script.rpy:301
    old "Rainbow Dash"
    new "¡Arco Iris! Un nombre… una promesa de luz en la noche. ¡Pero qué tormenta esconde su sonrisa! ¡Ay, Dios! Su presencia ilumina el mundo. ¡Arco Iris! ¡Su destino entrelazado al mío en un laberinto de pasión!"

    # game/Scripts/script.rpy:302
    old "Applejack"
    new "¡Applejack! ¡Ay, Dios, ese nombre… ese tormento! Como un rayo que ilumina y quema. ¡Un destino cruel me ha entregado este amor prohibido! ¡Mi corazón se estruja bajo esta sublime pasión!"

    # game/Scripts/script.rpy:303
    old "Rarity"
    new "¡Ay, Dios! ¡La Rareza! Un tesoro inalcanzable, una joya en las sombras. ¡Una visión fugaz, un sueño prohibido! Su valor se mide en la agonía del anhelo. ¡Cada instante sin ella es una eternidad! ¡La Rareza... un milagro efímero!"

    # game/Scripts/script.rpy:304
    old "Big Macintosh"
    new "¡Ay, Dios! ¡Big Macintosh! Su nombre resuena como un trueno de desesperación. ¡Un gigante de fuerza y silencio! ¿Cómo pudo el destino unir mi corazón a su sombra? ¡Es una cruel burla del universo!"

    # game/Scripts/script.rpy:532
    old "Twilight"
    new "¡Ay, Dios! *Crepúsculo*… una sombra sobre mi alma. ¡Un amor prohibido, oscuro como la muerte! Un corazón atormentado, dividido. ¡Mi alma llora! Una pasión que quema y amenaza con consumirme. ¡Ay, mi corazón! *Crepúsculo*… un grito ahogado en la eternidad."

    # game/Scripts/script.rpy:533
    old "Twi'"
    new "¡Ay, Dios! ¡Twi...! Un nombre que resuena como un trueno devastador. ¡No puede ser! ¿Cómo esa sílaba despierta tal tormenta de dolor? ¡Es una herida abierta! ¡Twi...!"

    # game/Scripts/script.rpy:537
    old "Pinkie"
    new "¡Ay, Dios! ¡Rosita! ¡Un nombre delicado como una flor marchita! ¡Un susurro de esperanza ahogado en sombras! ¡Su mención me desgarra el alma! ¡Rosita... un sueño desvanecido como rocío al sol!"

    # game/Scripts/script.rpy:538
    old "Pinkeh"
    new "¡Ay, Dios! ¡Pinkeh… una puñalada en mi alma! Una sombra de misterio. ¿Qué secretos guarda? ¡Mi corazón se estremece! ¡Pinkeh… una maldición, quizás!"

    # game/Scripts/script.rpy:543
    old "AJ"
    new "¡Ay, Dios! ¡A-A-Alejandro! ¿Es posible que mi corazón se rompa así? ¡Después de todo, me abandonas con este silencio! ¡Mi alma sangra, Alejandro! ¿No ves el abismo en mi ser?"

    # game/Scripts/script.rpy:547
    old "Rainbow"
    new "¡Ay, Dios! ¡Un arcoíris! ¡Una promesa de esperanza en el cielo! ¡Una visión celestial que me quiebra el alma! ¡Espejo de mis sueños más profundos! ¡No puede ser tan perfecto! ¡Me ahogo en su luz!"

    # game/Scripts/script.rpy:548
    old "Dashie"
    new "¡Ay, Dios! ¡Dashie...! Un nombre que resuena como un trueno devastador. ¿Qué tormenta se esconde tras ese sonido? ¡No puede ser! ¿El universo conspira para atormentarme? ¡Dashie...! Mi corazón sangra al pronunciarlo."

    # game/Scripts/script.rpy:558
    old "'Shy"
    new "¡Ay, Dios! Tímido… ¡Como un cervatillo asustado! Un alma delicada, encerrada en sí misma. ¡No puede ser! ¿Cómo alguien tan puro puede existir sin mostrar su luz? ¡Es una tragedia! ¡Una flor marchitándose!"

    # game/Scripts/script.rpy:560
    old "Wait!! I want to be my OC!! It's a super-awesome alicorn with neon colors and stuff!!!!"
    new "¡Espera! ¡Quiero ser mi Personaje Original! ¡Mi propia creación! ¡Es un alicorno... un ser de luz y magia, *espectacular*! ¡Con colores neón, vibrantes como el fuego! ¡Ay, Dios! ¡Es mi sueño más profundo!"

