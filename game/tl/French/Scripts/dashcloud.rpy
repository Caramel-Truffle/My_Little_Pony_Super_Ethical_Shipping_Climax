# TODO: Translation updated at 2025-11-26 12:28

# game/Scripts/dashcloud.rpy:8
translate French dashcloud_89abc92a:

    # "As a pegasus, you are easily able to fly to Rainbow's cloud."
    "En tant que pégase, vous pouvez facilement voler jusqu'au nuage de Rainbow."

# game/Scripts/dashcloud.rpy:9
translate French dashcloud_09477f07:

    # p "Wings are useful, [playername!t] was a good choice."
    p "Les ailes sont utiles, [playername!t] était un bon choix."

# game/Scripts/dashcloud.rpy:11
translate French dashcloud_817a4c8f:

    # "As a unicorn, you are able to cast spells to go up to Rainbow's cloud and not fall through it."
    "En tant que licorne, vous pouvez lancer des sorts pour monter au nuage de Rainbow et ne pas tomber à travers."

# game/Scripts/dashcloud.rpy:12
translate French dashcloud_859397e4:

    # p "Magic is useful, [playername!t] was a good choice."
    p "La magie est utile, [playername!t] était un bon choix."

# game/Scripts/dashcloud.rpy:14
translate French dashcloud_36385248:

    # "Anyway, Rainbow Dash is there, searching something under her bed, she apparently hasn't seen you yet."
    "Quoi qu'il en soit, Rainbow Dash est là, cherchant quelque chose sous son lit, elle ne vous a apparemment pas encore vu."

# game/Scripts/dashcloud.rpy:15
translate French dashcloud_5afbc557:

    # "What are you going to do?"
    "Qu'allez-vous faire?"

# game/Scripts/dashcloud.rpy:19
translate French datdashplot_dc115e3a:

    # "You look at her plot for a while, licking your lips while admiring her beautiful scenario. Ah, all the words and choices and-"
    "Vous regardez son arrière-train pendant un moment, vous léchant les lèvres en admirant son beau scénario. Ah, tous les mots et choix et-"

# game/Scripts/dashcloud.rpy:20
translate French datdashplot_f5ce8270:

    # "She grunts, as she apparently didn't find what she was looking for."
    "Elle grogne, car elle n'a apparemment pas trouvé ce qu'elle cherchait."

# game/Scripts/dashcloud.rpy:21
translate French datdashplot_e24b4d7b:

    # rd "What the hay? If I did not leave it there, where could it be?"
    rd "Mais qu'est-ce que c'est que ça? Si je ne l'ai pas laissé là, où est-ce que ça peut être?"

# game/Scripts/dashcloud.rpy:23
translate French datdashplot_fea1e80a:

    # p "Hey Rainbow, need any help?"
    p "Hé Rainbow, besoin d'aide?"

# game/Scripts/dashcloud.rpy:24
translate French datdashplot_d9e91ea5:

    # rd "Sure, do you- Wait. What?"
    rd "Bien sûr, est-ce que tu- Attends. Quoi?"

# game/Scripts/dashcloud.rpy:25
translate French datdashplot_7dd30603:

    # "She stands up, turn around and sees you."
    "Elle se lève, se retourne et vous voit."

# game/Scripts/dashcloud.rpy:30
translate French datdashplot_b069318f:

    # rd "Hey, I didn't hear you coming!"
    rd "Hé, je ne t'ai pas entendu arriver!"

# game/Scripts/dashcloud.rpy:34
translate French datdashplot_0082e58e:

    # rd "But it's always a pleasure to welcome true fans to my lair!"
    rd "Mais c'est toujours un plaisir d'accueillir de vrais fans dans mon repaire!"

# game/Scripts/dashcloud.rpy:35
translate French datdashplot_a5e14cce:

    # p "True fans? ...Lair?"
    p "Vrais fans? ...Repaire?"

# game/Scripts/dashcloud.rpy:36
translate French datdashplot_291020bc:

    # rd "Yeah, look at you! You are like my clone! You even got my cutie mark right! And my rainbow-cloud. I'm not the heroin this city needs, but the one this city deserves!"
    rd "Ouais, regarde-toi! Tu es comme mon clone! Tu as même ma cutie mark correctement! Et mon nuage arc-en-ciel. Je ne suis pas l'héroïne dont cette ville a besoin, mais celle qu'elle mérite!"

# game/Scripts/dashcloud.rpy:38
translate French datdashplot_bc4cd5d5:

    # p "You watched batmare too much."
    p "Tu as trop regardé Batmare."

# game/Scripts/dashcloud.rpy:40
translate French datdashplot_99551c5e:

    # rd "But she's so awesome! ...Hey, I did not catch your name."
    rd "Mais elle est tellement géniale! ...Hé, je n'ai pas attrapé ton nom."

# game/Scripts/dashcloud.rpy:41
translate French datdashplot_b4af8ed3:

    # p "Can you make one of my dream come true and call me Rainbow Dash?"
    p "Peux-tu réaliser un de mes rêves et m'appeler Rainbow Dash?"

# game/Scripts/dashcloud.rpy:42
translate French datdashplot_724d5b96:

    # rd "You really are a true fan! I like that! Okay, but only for today. Tomorrow I'll call you by your real name, deal?"
    rd "Tu es vraiment un vrai fan! J'aime ça! D'accord, mais seulement pour aujourd'hui. Demain je t'appellerai par ton vrai nom, d'accord?"

# game/Scripts/dashcloud.rpy:43
translate French datdashplot_31a90cd2:

    # p "Deal."
    p "D'accord."

# game/Scripts/dashcloud.rpy:47
translate French datdashplot_f2278cf7:

    # rd "Are you just passing by [playername3!t], or do you need something?"
    rd "Tu ne fais que passer [playername3!t], ou tu as besoin de quelque chose?"

# game/Scripts/dashcloud.rpy:48
translate French datdashplot_b146c7b8:

    # p "I was wondering if you could help me with something."
    p "Je me demandais si tu pouvais m'aider avec quelque chose."

# game/Scripts/dashcloud.rpy:51
translate French datdashplot_20f4f91e:

    # rd "Do you need help in the library? I heard it's this time of the year where you sort all the books again."
    rd "Tu as besoin d'aide à la bibliothèque? J'ai entendu dire que c'est cette période de l'année où tu tries tous les livres à nouveau."

# game/Scripts/dashcloud.rpy:53
translate French datdashplot_e07f6bc8:

    # rd "Please tell me you are not looking for a model to design a dress!"
    rd "S'il te plaît, dis-moi que tu ne cherches pas un mannequin pour dessiner une robe!"

# game/Scripts/dashcloud.rpy:55
translate French datdashplot_cd3fbfe1:

    # rd "Do you need some help at night? It would be a pleasure."
    rd "Tu as besoin d'aide la nuit? Ce serait un plaisir."

# game/Scripts/dashcloud.rpy:57
translate French datdashplot_1af79adf:

    # rd "And what can the awesome Rainbow Dash do for you?"
    rd "Et que peut faire la géniale Rainbow Dash pour toi?"

# game/Scripts/dashcloud.rpy:59
translate French datdashplot_2d4a9ba7:

    # p "In fact, I was thinking of asking help..."
    p "En fait, je pensais demander de l'aide..."

# game/Scripts/dashcloud.rpy:63
translate French askdashiehelp_8ca08919:

    # p "...to be cooler."
    p "...pour être plus cool."

# game/Scripts/dashcloud.rpy:64
translate French askdashiehelp_f92adb54:

    # rd "Cooler?"
    rd "Plus cool?"

# game/Scripts/dashcloud.rpy:67
translate French askdashiehelp_653097b6:

    # p "I need to be about 20%% cooler."
    p "J'ai besoin d'être environ 20%% plus cool."

# game/Scripts/dashcloud.rpy:68
translate French askdashiehelp_035770f1:

    # rd "That sentence has been overused, you know?"
    rd "Cette phrase a été trop utilisée, tu sais?"

# game/Scripts/dashcloud.rpy:69
translate French askdashiehelp_c26d4b78:

    # p "Sorry. Anyway, do you think you could help?"
    p "Désolé. En tout cas, tu penses que tu pourrais aider?"

# game/Scripts/dashcloud.rpy:74
translate French askdashiehelp_1ed89f17:

    # rd "We could make out. I already told you that I was open with that."
    rd "On pourrait s'embrasser. Je t'ai déjà dit que j'étais ouverte à ça."

# game/Scripts/dashcloud.rpy:75
translate French askdashiehelp_d5a39c7d:

    # p "That isn't what I asked!"
    p "Ce n'est pas ce que j'ai demandé!"

# game/Scripts/dashcloud.rpy:76
translate French askdashiehelp_76a0385d:

    # rd "Does that mean that you are declining my offer again?"
    rd "Est-ce que ça veut dire que tu refuses mon offre encore?"

# game/Scripts/dashcloud.rpy:80
translate French easyflutterdash_c5ddb242:

    # p "Yes. What could you do to make me cooler instead?"
    p "Oui. Que pourrais-tu faire pour me rendre plus cool à la place?"

# game/Scripts/dashcloud.rpy:82
translate French easyflutterdash_6b1e1e97:

    # p "Well... Erm... No."
    p "Eh bien... Euh... Non."

# game/Scripts/dashcloud.rpy:85
translate French easyflutterdash_a71491a4:

    # rd "Really? That's gonna be awesome! Thanks Fluttershy!"
    rd "Vraiment? Ça va être génial! Merci Fluttershy!"

# game/Scripts/dashcloud.rpy:86
translate French easyflutterdash_f628511e:

    # p "Just... Be gentle, please."
    p "Juste... Sois douce, s'il te plaît."

# game/Scripts/dashcloud.rpy:91
translate French easyflutterdash_f02d2de8:

    # "And then both of you did naughty things, the bed being too close to ignore it."
    "Et puis vous avez tous les deux fait des choses coquines, le lit étant trop proche pour l'ignorer."

# game/Scripts/dashcloud.rpy:92
translate French easyflutterdash_491425f7:

    # p "Your lack of pictures on that matter disappoint me."
    p "Votre manque d'images à ce sujet me déçoit."

# game/Scripts/dashcloud.rpy:93
translate French easyflutterdash_c0e9c353:

    # "Tell that to the ghost visual artist, because the best I can do is this."
    "Dis ça à l'artiste visuel fantôme, parce que le mieux que je puisse faire c'est ça."

# game/Scripts/dashcloud.rpy:94
translate French easyflutterdash_a2a6d5b0:

    # p "If you could do anything, what would you choose?"
    p "Si tu pouvais faire n'importe quoi, que choisirais-tu?"

# game/Scripts/dashcloud.rpy:95
translate French easyflutterdash_f3d221c5:

    # "Chibi ponies kissing each other in a kawaii way. This isn't a 18+ game after all."
    "Des poneys chibi qui s'embrassent de manière kawaii. Ce n'est pas un jeu 18+ après tout."

# game/Scripts/dashcloud.rpy:96
translate French easyflutterdash_e85ddcd3:

    # p "One day..."
    p "Un jour..."

# game/Scripts/dashcloud.rpy:97
translate French easyflutterdash_0a83f1c8:

    # "The chibis. Not the change of rating. When I said they did naughty things, I was talking about eating.... IN THE BED! Think about it! All the crumbs everywere! Naughty! Filthy!"
    "Les chibis. Pas le changement de classification. Quand j'ai dit qu'ils ont fait des choses coquines, je parlais de manger.... DANS LE LIT! Pensez-y! Toutes les miettes partout! Coquin! Sale!"

# game/Scripts/dashcloud.rpy:99
translate French easyflutterdash_74ef33bb:

    # "--Rainbow ending 1--"
    "--Fin Rainbow 1--"

# game/Scripts/dashcloud.rpy:103
translate French easyflutterdash_05654b16:

    # rd "Well, there's one easy way to handle that. I can't guarantee its results though."
    rd "Eh bien, il y a un moyen facile de gérer ça. Je ne peux pas garantir ses résultats cependant."

# game/Scripts/dashcloud.rpy:104
translate French easyflutterdash_d1047b0e:

    # p "And what would it be?"
    p "Et ce serait quoi?"

# game/Scripts/dashcloud.rpy:106
translate French easyflutterdash_a364db8b:

    # rd "Let me put you in context, first. Imagine you wanted to seem nicer. You know the principle of the lovely item, right? Something cute that makes you look like a nice pony?"
    rd "Laisse-moi te mettre en contexte d'abord. Imagine que tu voulais paraître plus gentil. Tu connais le principe de l'objet charmant, non? Quelque chose de mignon qui te fait ressembler à un gentil poney?"

# game/Scripts/dashcloud.rpy:107
translate French easyflutterdash_2d1433f4:

    # p "Like a plushie or a young kid?"
    p "Comme une peluche ou un jeune enfant?"

# game/Scripts/dashcloud.rpy:108
translate French easyflutterdash_a0838bfc:

    # rd "Yeah. There's the same thing to look cooler instead of nicer, a \"cool item\" if you want."
    rd "Ouais. Il y a la même chose pour paraître plus cool au lieu de plus gentil, un \"objet cool\" si tu veux."

# game/Scripts/dashcloud.rpy:109
translate French easyflutterdash_2f733bb2:

    # p "And you have a cool item I could borrow?"
    p "Et tu as un objet cool que je pourrais emprunter?"

# game/Scripts/dashcloud.rpy:110
translate French easyflutterdash_f81b4f06:

    # rd "Of course. Wait there."
    rd "Bien sûr. Attends là."

# game/Scripts/dashcloud.rpy:114
translate French easyflutterdash_5b477f39:

    # "She went somewhere else in her cloud, looking for something, before coming back with a pair of sunglasses."
    "Elle est allée ailleurs dans son nuage, cherchant quelque chose, avant de revenir avec une paire de lunettes de soleil."

# game/Scripts/dashcloud.rpy:120
translate French easyflutterdash_71bc14f0:

    # rd "There, the coolest glasses in all of Equestria!"
    rd "Voilà, les lunettes les plus cool de toute Equestria!"

# game/Scripts/dashcloud.rpy:122
translate French easyflutterdash_9d4f8f5e:

    # p "Thank you Dashie! You are a life saver!"
    p "Merci Dashie! Tu me sauves la vie!"

# game/Scripts/dashcloud.rpy:124
translate French easyflutterdash_8c980dbd:

    # "You receive a cool pair of sunglasses!"
    "Vous recevez une paire de lunettes de soleil cool!"

# game/Scripts/dashcloud.rpy:125
translate French easyflutterdash_3c47e800:

    # rd "Well... I hope you'll tell me what this is all about tomorrow."
    rd "Eh bien... J'espère que tu me diras de quoi il s'agit demain."

# game/Scripts/dashcloud.rpy:126
translate French easyflutterdash_1372ef6b:

    # p "Tomorrow?"
    p "Demain?"

# game/Scripts/dashcloud.rpy:127
translate French easyflutterdash_741ca70e:

    # rd "Yeah, it was fun and all, but I promised Derpy I would go train myself with her today, so I'm going to be busy for the rest of the day."
    rd "Ouais, c'était amusant et tout, mais j'ai promis à Derpy que j'irais m'entraîner avec elle aujourd'hui, donc je vais être occupée pour le reste de la journée."

# game/Scripts/dashcloud.rpy:128
translate French easyflutterdash_01c722f7:

    # p "You are really awesome."
    p "Tu es vraiment géniale."

# game/Scripts/dashcloud.rpy:130
translate French easyflutterdash_1d0ca91b:

    # rd "Yeah, yeah... But, please, don't go near us with these, I'll explain to you tomorrow."
    rd "Ouais, ouais... Mais, s'il te plaît, ne t'approche pas de nous avec ça, je t'expliquerai demain."

# game/Scripts/dashcloud.rpy:131
translate French easyflutterdash_c11adbeb:

    # p "No problem, see you tomorrow!"
    p "Pas de problème, à demain!"

# game/Scripts/dashcloud.rpy:132
translate French easyflutterdash_1fe2d40c:

    # rd "See ya!"
    rd "À plus!"

# game/Scripts/dashcloud.rpy:136
translate French easyflutterdash_59d81184:

    # "And she invited you to leave before doing so herself, then you went back to the Ponyville crossroad, with her pair of cool sunglasses in... Wait... How do you even carry the items that ponies give you?"
    "Et elle vous a invité à partir avant de le faire elle-même, puis vous êtes retourné au carrefour de Ponyville, avec sa paire de lunettes de soleil cool dans... Attends... Comment portez-vous même les objets que les poneys vous donnent?"

# game/Scripts/dashcloud.rpy:138
translate French easyflutterdash_b53e7692:

    # p "You don't want to know."
    p "Vous ne voulez pas savoir."

# game/Scripts/dashcloud.rpy:139
translate French easyflutterdash_fc5301a2:

    # "I heard nothing."
    "Je n'ai rien entendu."

# game/Scripts/dashcloud.rpy:143
translate French easyflutterdash_19902319:

    # p "...to gain some strength."
    p "...pour gagner de la force."

# game/Scripts/dashcloud.rpy:145
translate French easyflutterdash_bec1adeb:

    # rd "Strength? Really?"
    rd "De la force? Vraiment?"

# game/Scripts/dashcloud.rpy:146
translate French easyflutterdash_c610938f:

    # p "Yes, I may not be an earth pony, but healthy exercises can't be ignored."
    p "Oui, je ne suis peut-être pas un poney terrestre, mais les exercices sains ne peuvent pas être ignorés."

# game/Scripts/dashcloud.rpy:148
translate French easyflutterdash_2faa68c9:

    # rd "Yeah, no, that was the timing that surprised me, I was searching a dumbbell to go train myself with Derpy, because she asked me to too!"
    rd "Ouais, non, c'est le timing qui m'a surpris, je cherchais un haltère pour aller m'entraîner avec Derpy, parce qu'elle me l'a demandé aussi!"

# game/Scripts/dashcloud.rpy:149
translate French easyflutterdash_ae2d537e:

    # p "Really? Can I join you?"
    p "Vraiment? Je peux me joindre à vous?"

# game/Scripts/dashcloud.rpy:151
translate French easyflutterdash_9ddaf9d0:

    # rd "I don't think she would mind, so, yeah, sure! But first, we need to find that dumbbell."
    rd "Je ne pense pas que ça la dérangerait, donc, ouais, bien sûr! Mais d'abord, nous devons trouver cet haltère."

# game/Scripts/dashcloud.rpy:152
translate French easyflutterdash_68b25803:

    # p "Isn't it the one just right there, on the table right in front of you?"
    p "N'est-ce pas celui juste là, sur la table juste devant toi?"

# game/Scripts/dashcloud.rpy:155
translate French easyflutterdash_efb20066:

    # rd ".....\n .....\n Nooooo. Not at all, you can take this one. This... Isn't the one at all, I'm not stupid."
    rd ".....\n .....\n Nooooon. Pas du tout, tu peux prendre celui-ci. Ce... N'est pas du tout celui-là, je ne suis pas stupide."

# game/Scripts/dashcloud.rpy:159
translate French easyflutterdash_3ea3283e:

    # "While blushing a bit, Rainbow Dash gives you a dumbbell!"
    "En rougissant un peu, Rainbow Dash vous donne un haltère!"

# game/Scripts/dashcloud.rpy:161
translate French easyflutterdash_33c5e109:

    # rd "I will... Erm. Continue to search. Go back here as soon as you are ready to go."
    rd "Je vais... Euh. Continuer à chercher. Reviens ici dès que tu es prêt à partir."

# game/Scripts/dashcloud.rpy:162
translate French easyflutterdash_3ccab5bb:

    # p "Ooookay... See you later, then."
    p "Ooookay... À plus tard, alors."

# game/Scripts/dashcloud.rpy:163
translate French easyflutterdash_e42f06f5:

    # rd "Yeah, real soon!"
    rd "Ouais, très bientôt!"

# game/Scripts/dashcloud.rpy:167
translate French easyflutterdash_100342ca:

    # "And so you were back on your way to the crossroad, with a bit of extra weight."
    "Et donc vous étiez de retour sur le chemin du carrefour, avec un peu de poids supplémentaire."

# game/Scripts/dashcloud.rpy:177
translate French easyflutterdash_ccb93bbd:

    # "You are back to Rainbow's cloud, but instead of her, you stumble upon Derpy Hooves, a gray mare with yellow... Eyes."
    "Vous êtes de retour au nuage de Rainbow, mais au lieu d'elle, vous tombez sur Derpy Hooves, une jument grise avec des yeux... jaunes."

# game/Scripts/dashcloud.rpy:180
translate French easyflutterdash_ca9cd8f3:

    # d "Rainbow Dash! You are back!"
    d "Rainbow Dash! Tu es de retour!"

# game/Scripts/dashcloud.rpy:184
translate French derpyabuse_e2663b37:

    # p "I'm not Rainbow Dash, I'm just... A fan. Yeah, a hardcore fan, cosplaying as her and all."
    p "Je ne suis pas Rainbow Dash, je suis juste... Un fan. Ouais, un fan hardcore, qui fait du cosplay d'elle et tout."

# game/Scripts/dashcloud.rpy:185
translate French derpyabuse_fb11b1df:

    # d "Cool! I like her too! And muffins! I'm Derpy Hooves, what's your name?"
    d "Cool! Je l'aime aussi! Et les muffins! Je suis Derpy Hooves, quel est ton nom?"

# game/Scripts/dashcloud.rpy:186
translate French derpyabuse_6e7ca0fc:

    # p "Well, why don't you call me Rainbow Dash as long as I'm cosplaying?"
    p "Eh bien, pourquoi ne m'appelles-tu pas Rainbow Dash tant que je fais du cosplay?"

# game/Scripts/dashcloud.rpy:187
translate French derpyabuse_c25764a8:

    # d "I muffin this idea!"
    d "Je muffin cette idée!"

# game/Scripts/dashcloud.rpy:188
translate French derpyabuse_9043e234:

    # p "Muffin?"
    p "Muffin?"

# game/Scripts/dashcloud.rpy:189
translate French derpyabuse_ba22310f:

    # d "I love muffins!"
    d "J'adore les muffins!"

# game/Scripts/dashcloud.rpy:193
translate French truederpyabuse_815c8d68:

    # p "I have something even more delicious for you..."
    p "J'ai quelque chose d'encore plus délicieux pour toi..."

# game/Scripts/dashcloud.rpy:194
translate French truederpyabuse_cbcfbf54:

    # d "Really? Even more than muffins? That's not possible!"
    d "Vraiment? Encore plus que les muffins? Ce n'est pas possible!"

# game/Scripts/dashcloud.rpy:195
translate French truederpyabuse_0d3c0b9a:

    # p "Oh yes it is, close your eyes..."
    p "Oh si ça l'est, ferme tes yeux..."

# game/Scripts/dashcloud.rpy:201
translate French truederpyabuse_3c815e70:

    # "You kissed her."
    "Vous l'avez embrassée."

# game/Scripts/dashcloud.rpy:209
translate French truederpyabuse_e166e62e:

    # d "Kisses and hugs time!"
    d "C'est l'heure des bisous et des câlins!"

# game/Scripts/dashcloud.rpy:216
translate French truederpyabuse_d5aa4191:

    # "And it was kisses and hugs time for the remaining of the day, Dashie joigning you once she came back, thinking it was a suitable replacement for the training session she was initially planning."
    "Et c'était l'heure des bisous et des câlins pour le reste de la journée, Dashie vous rejoignant une fois qu'elle est revenue, pensant que c'était un remplacement approprié pour la session d'entraînement qu'elle prévoyait initialement."

# game/Scripts/dashcloud.rpy:217
translate French truederpyabuse_f10a4ec1:

    # p "Three mares \"kissing and hugging\"? That's all I get? On a still image like this one?"
    p "Trois juments qui \"s'embrassent et se câlinent\"? C'est tout ce que j'obtiens? Sur une image fixe comme celle-ci?"

# game/Scripts/dashcloud.rpy:218
translate French truederpyabuse_40202db6:

    # "There are other games if you want graphic action."
    "Il y a d'autres jeux si vous voulez de l'action graphique."

# game/Scripts/dashcloud.rpy:220
translate French truederpyabuse_33b443f6:

    # "--Derpy ending--"
    "--Fin Derpy--"

# game/Scripts/dashcloud.rpy:224
translate French truederpyabuse_248944c5:

    # p "I love them too! But, have you seen the real Rainbow Dash?"
    p "Je les adore aussi! Mais, as-tu vu la vraie Rainbow Dash?"

# game/Scripts/dashcloud.rpy:227
translate French truederpyabuse_e8b417cd:

    # p "Yeah, get on my bed and close your eyes, I'm gonna do something awesome!"
    p "Ouais, monte sur mon lit et ferme tes yeux, je vais faire quelque chose de génial!"

# game/Scripts/dashcloud.rpy:228
translate French truederpyabuse_c6edeebb:

    # d "Rainbow Dash? Are you alright?"
    d "Rainbow Dash? Ça va?"

# game/Scripts/dashcloud.rpy:231
translate French truederpyabuse_a9d49315:

    # rd "What are you doing?"
    rd "Qu'est-ce que tu fais?"

# game/Scripts/dashcloud.rpy:232
translate French truederpyabuse_6da5b00d:

    # p "Where did you come from?!"
    p "D'où viens-tu?!"

# game/Scripts/dashcloud.rpy:233
translate French truederpyabuse_abbefd25:

    # rd "Doesn't matter. You come to my house. Pretend to be me. And try to abuse my friends? GET OUT!"
    rd "Peu importe. Tu viens chez moi. Tu prétends être moi. Et tu essaies d'abuser de mes amis? SORS!"

# game/Scripts/dashcloud.rpy:234
translate French truederpyabuse_94e84c43:

    # p "But-"
    p "Mais-"

# game/Scripts/dashcloud.rpy:240
translate French truederpyabuse_be0705e6:

    # "And she kicked you so hard... You just lost the game."
    "Et elle vous a donné un coup de pied si fort... Vous venez de perdre le jeu."

# game/Scripts/dashcloud.rpy:241
translate French truederpyabuse_a4d9c224:

    # p "This isn't funny anymore either."
    p "Ce n'est plus drôle non plus."

# game/Scripts/dashcloud.rpy:242
translate French truederpyabuse_68799c97:

    # "You lost anyway."
    "Vous avez perdu de toute façon."

# game/Scripts/dashcloud.rpy:244
translate French truederpyabuse_fd868e06:

    # "--Rainbow Dash ending 2--"
    "--Fin Rainbow Dash 2--"

# game/Scripts/dashcloud.rpy:247
translate French truederpyabuse_48911279:

    # d "Hey [playername2!t]! Rainbow Dash and me were going to go train for a bit, do you want to join us?"
    d "Hé [playername2!t]! Rainbow Dash et moi allions nous entraîner un peu, veux-tu te joindre à nous?"

# game/Scripts/dashcloud.rpy:248
translate French truederpyabuse_e9f19837:

    # p "Actually, that's what I'm here for, Derpy. But, where's Rainbow?"
    p "En fait, c'est pour ça que je suis là, Derpy. Mais, où est Rainbow?"

# game/Scripts/dashcloud.rpy:249
translate French truederpyabuse_4fd0a61e:

    # d "I just don't know where she went."
    d "Je ne sais vraiment pas où elle est allée."

# game/Scripts/dashcloud.rpy:250
translate French truederpyabuse_b91fd848:

    # p "I expected you to say something similar. Anyway, she will surely be back soon."
    p "Je m'attendais à ce que tu dises quelque chose de similaire. En tout cas, elle sera sûrement de retour bientôt."

# game/Scripts/dashcloud.rpy:251
translate French truederpyabuse_8af0bad0:

    # d "That's true [playername2!t]! And-"
    d "C'est vrai [playername2!t]! Et-"

# game/Scripts/dashcloud.rpy:254
translate French truederpyabuse_94418b3c:

    # d "I... Lost my Muffin..."
    d "J'ai... Perdu mon Muffin..."

# game/Scripts/dashcloud.rpy:255
translate French truederpyabuse_78a79987:

    # p "What? Where?"
    p "Quoi? Où?"

# game/Scripts/dashcloud.rpy:256
translate French truederpyabuse_5aaaf352:

    # d "I don't know, I simply wanted to eat one, but remembered it was my last one, but decided to eat it anyway, but... I can't find it anymore!"
    d "Je ne sais pas, je voulais simplement en manger un, mais je me suis souvenue que c'était mon dernier, mais j'ai décidé de le manger quand même, mais... Je ne le trouve plus!"

# game/Scripts/dashcloud.rpy:257
translate French truederpyabuse_b4e6dcb6:

    # p "Well, we will buy you some, the Sugarcube Corner isn't far."
    p "Eh bien, nous t'en achèterons, le Sugarcube Corner n'est pas loin."

# game/Scripts/dashcloud.rpy:258
translate French truederpyabuse_26404882:

    # d "That was their last one for today, they have a production problem and won't be able to make some before tomorrow..."
    d "C'était leur dernier pour aujourd'hui, ils ont un problème de production et ne pourront pas en faire avant demain..."

# game/Scripts/dashcloud.rpy:259
translate French truederpyabuse_70e3593e:

    # p "You really want that muffin, don't you?"
    p "Tu veux vraiment ce muffin, n'est-ce pas?"

# game/Scripts/dashcloud.rpy:260
translate French truederpyabuse_b2842d1b:

    # d "Yes..."
    d "Oui..."

# game/Scripts/dashcloud.rpy:263
translate French truederpyabuse_3bb97d7b:

    # p "Then take this one, Pinkie gave it to me earlier."
    p "Alors prends celui-ci, Pinkie me l'a donné plus tôt."

# game/Scripts/dashcloud.rpy:268
translate French truederpyabuse_521da80b:

    # p "Wait there, I'll get you one. I don't know how, but I'll get you one."
    p "Attends là, je vais t'en trouver un. Je ne sais pas comment, mais je vais t'en trouver un."

# game/Scripts/dashcloud.rpy:271
translate French truederpyabuse_ce787523:

    # d "Thank you [playername2!t], I will wait for your return."
    d "Merci [playername2!t], j'attendrai ton retour."

# game/Scripts/dashcloud.rpy:272
translate French truederpyabuse_6e814c8c:

    # p "I will come back as soon as possible!"
    p "Je reviendrai dès que possible!"

# game/Scripts/dashcloud.rpy:278
translate French truederpyabuse_f8b8fd6c:

    # "Without a clue, you went back to the crossroad."
    "Sans indice, vous êtes retourné au carrefour."

# game/Scripts/dashcloud.rpy:285
translate French truederpyabuse_1aad1906:

    # "You go back to Rainbow's cloud, where Derpy Hooves is waiting for a muffin."
    "Vous retournez au nuage de Rainbow, où Derpy Hooves attend un muffin."

# game/Scripts/dashcloud.rpy:288
translate French truederpyabuse_2a4fc9d0:

    # d "Hey [playername2!t], you are back! Did you find any muffin?"
    d "Hé [playername2!t], tu es de retour! As-tu trouvé un muffin?"

# game/Scripts/dashcloud.rpy:290
translate French truederpyabuse_957ef681:

    # p "Hey Derpy! Sadly no, I haven't found a muffin yet."
    p "Hé Derpy! Malheureusement non, je n'ai pas encore trouvé de muffin."

# game/Scripts/dashcloud.rpy:291
translate French truederpyabuse_863a77e1:

    # d "Yet? Are you going to try again?"
    d "Encore? Tu vas réessayer?"

# game/Scripts/dashcloud.rpy:292
translate French truederpyabuse_56d02e78:

    # p "Of course I will! And I will be back as soon as I can!"
    p "Bien sûr que oui! Et je reviendrai dès que je pourrai!"

# game/Scripts/dashcloud.rpy:293
translate French truederpyabuse_501137be:

    # d "Take care..."
    d "Prends soin de toi..."
translate French strings:

    # game/Scripts/dashcloud.rpy:17
    old "Stare at her plot"
    new "Fixer son arrière-train"

    # game/Scripts/dashcloud.rpy:18
    old "Offer your help"
    new "Offrir votre aide"

    # game/Scripts/dashcloud.rpy:61
    old "To be cooler"
    new "Pour être plus cool"

    # game/Scripts/dashcloud.rpy:62
    old "To gain some strength"
    new "Pour gagner de la force"

    # game/Scripts/dashcloud.rpy:78
    old "Decline the offer"
    new "Refuser l'offre"

    # game/Scripts/dashcloud.rpy:79
    old "Accept the offer"
    new "Accepter l'offre"

    # game/Scripts/dashcloud.rpy:182
    old "Tell her the truth"
    new "Lui dire la vérité"

    # game/Scripts/dashcloud.rpy:183
    old "Take advantage of the situation"
    new "Profiter de la situation"

    # game/Scripts/dashcloud.rpy:191
    old "\"I have a very special muffin for you...\""
    new "\"J'ai un muffin très spécial pour toi...\""

    # game/Scripts/dashcloud.rpy:192
    old "\"I love them too!\""
    new "\"Je les adore aussi!\""
# TODO: Translation updated at 2025-11-28 10:15

# game/Scripts/dashcloud.rpy:297
translate French truederpyabuse_7d44d21d:

    # "And you went back to the crossroad. Where could you find a muffin?"
    "Et vous êtes retourné au carrefour. Où pourriez-vous trouver un muffin ?"

# game/Scripts/dashcloud.rpy:300
translate French truederpyabuse_a37d917d:

    # p "Hey Derpy! Of course I do, take this one!"
    p "Hé Derpy ! Bien sûr que j'en ai, prends celui-ci !"

# game/Scripts/dashcloud.rpy:304
translate French yummymuffin_60b6fe2f:

    # "You give your muffin to Derpy."
    "Vous donnez votre muffin à Derpy."

# game/Scripts/dashcloud.rpy:305
translate French yummymuffin_a74114b3:

    # d "Thank you so much [playername2!t]!"
    d "Merci beaucoup [playername2!t] !"

# game/Scripts/dashcloud.rpy:313
translate French yummymuffin_0384e70d:

    # rd "Sorry girls, I took my time! ...[playername3!t], what did you do to Derpy? She's so happy!"
    rd "Désolée les filles, j'ai pris mon temps ! ...[playername3!t], qu'as-tu fait à Derpy ? Elle est si heureuse !"

# game/Scripts/dashcloud.rpy:316
translate French yummymuffin_af2f5e46:

    # d "She gave me a muffin! Muffin!"
    d "Elle m'a donné un muffin ! Muffin !"

# game/Scripts/dashcloud.rpy:319
translate French yummymuffin_5f233678:

    # p "She really does love them."
    p "Elle les aime vraiment."

# game/Scripts/dashcloud.rpy:320
translate French yummymuffin_251aca32:

    # rd "Well, anyway, let's go, what do you think?"
    rd "Eh bien, de toute façon, allons-y, qu'en penses-tu ?"

# game/Scripts/dashcloud.rpy:322
translate French yummymuffin_4fede62e:

    # d "Allons-y!"
    d "Allons-y !"

# game/Scripts/dashcloud.rpy:323
translate French yummymuffin_59ee328c:

    # rd "You've been around that stallion too much, Derpy."
    rd "Tu as trop fréquenté cet étalon, Derpy."

# game/Scripts/dashcloud.rpy:324
translate French yummymuffin_a1b5d9ee:

    # d "But he's so nice to me, like you two!"
    d "Mais il est si gentil avec moi, comme vous deux !"

# game/Scripts/dashcloud.rpy:325
translate French yummymuffin_c67e549a:

    # "And all three of you continued to chit-chat while going out."
    "Et vous trois avez continué à bavarder en sortant."

# game/Scripts/dashcloud.rpy:329
translate French yummymuffin_c4adac6b:

    # "And you three went outside ponyville to go train for a bit, lifting weights, running, flying, etc. After that, you waved goodbye to the two others -who were going back to their home- and returned to the crossroad."
    "Et vous trois êtes allés à l'extérieur de Ponyville pour vous entraîner un peu, soulever des poids, courir, voler, etc. Après cela, vous avez dit au revoir aux deux autres -qui retournaient chez elles- et êtes retourné au carrefour."

# game/Scripts/dashcloud.rpy:340
translate French yummymuffin_0adfd453:

    # "You come back to the Rainbow lair. The young athlete is here, drinking some water while sweat is beading off her fur."
    "Vous revenez au repaire de Rainbow. La jeune athlète est là, buvant de l'eau tandis que la sueur perle sur sa fourrure."

# game/Scripts/dashcloud.rpy:344
translate French yummymuffin_c1d57a63:

    # rd "Hi there! I was about to hit the shower, wanna join me?"
    rd "Salut ! J'étais sur le point de prendre une douche, tu veux te joindre à moi ?"

# game/Scripts/dashcloud.rpy:346
translate French yummymuffin_162f6dc7:

    # p "Sure thing!"
    p "Bien sûr !"

# game/Scripts/dashcloud.rpy:352
translate French yummymuffin_afde9838:

    # "And you two entered the shower, big enough for two adult ponies to get into and fully wash themselves without touching one another, because it's good to save water, but it's better to do it while respecting personal spaces."
    "Et vous deux êtes entrés dans la douche, assez grande pour que deux poneys adultes puissent y entrer et se laver entièrement sans se toucher, car c'est bien d'économiser l'eau, mais c'est mieux de le faire en respectant les espaces personnels."

# game/Scripts/dashcloud.rpy:354
translate French yummymuffin_390a9618:

    # p "I was wondering about something. Why does no one here has genitals?"
    p "Je me posais une question. Pourquoi personne ici n'a d'organes génitaux ?"

# game/Scripts/dashcloud.rpy:356
translate French yummymuffin_70dc61b7:

    # rd "No genius aliens? That sounds kind of racist."
    rd "Pas d'extraterrestres génies ? Ça semble un peu raciste."

# game/Scripts/dashcloud.rpy:357
translate French yummymuffin_ae7b114b:

    # p "No, I mean, everypony resembles a barbie doll down there."
    p "Non, je veux dire, tout le monde ressemble à une poupée Barbie en bas."

# game/Scripts/dashcloud.rpy:359
translate French yummymuffin_5dd7cf6c:

    # rd "Barbier dolls? You're making less and less sense by the minute."
    rd "Poupées Barbier ? Tu as de moins en moins de sens à chaque minute."

# game/Scripts/dashcloud.rpy:361
translate French yummymuffin_e0d0f471:

    # p "Rainbow Dash. How are fillies and colt made?"
    p "Rainbow Dash. Comment sont faits les pouliches et les poulains ?"

# game/Scripts/dashcloud.rpy:363
translate French yummymuffin_fbc37a88:

    # rd "You really don't know? We write a letter to the guys in charge of the script and ask them to add us a kid. Simple as that. Wanna know where to send your letter? I got the address noted down, in case I change my mind about children at some point."
    rd "Tu ne sais vraiment pas ? On écrit une lettre aux gars en charge du scénario et on leur demande de nous ajouter un enfant. Aussi simple que ça. Tu veux savoir où envoyer ta lettre ? J'ai l'adresse notée, au cas où je changerais d'avis sur les enfants à un moment donné."

# game/Scripts/dashcloud.rpy:365
translate French yummymuffin_ec2dc706:

    # p "Oof."
    p "Ouf."

# game/Scripts/dashcloud.rpy:367
translate French yummymuffin_5143d28f:

    # "Nice try updating your lingo. You two ended up having some nice clean fun during the afternoon, since you discovered the big secret explaining why this game is necessarily PG."
    "Bien essayé de mettre à jour ton jargon. Vous deux avez fini par vous amuser proprement pendant l'après-midi, puisque vous avez découvert le grand secret expliquant pourquoi ce jeu est nécessairement PG."

# game/Scripts/dashcloud.rpy:369
translate French yummymuffin_9421942c:

    # "Or whatever your classification system is. Seems like merely mentioning sexual intercourse is worthy of a Mature rating in some countries, while for others it's barely a 7+ thing. "
    "Ou quel que soit votre système de classification. Il semble que le simple fait de mentionner des rapports sexuels mérite une classification Mature dans certains pays, alors que pour d'autres, c'est à peine une chose 7+. "

# game/Scripts/dashcloud.rpy:371
translate French yummymuffin_9615ee76:

    # "Good job, Australia."
    "Bien joué, l'Australie."

# game/Scripts/dashcloud.rpy:373
translate French yummymuffin_9fbb2def:

    # p "Please just roll the credits."
    p "S'il vous plaît, faites défiler le générique."

# game/Scripts/dashcloud.rpy:395
translate French yummymuffin_08183c2d:

    # "--Rainbow Dash true ending--"
    "--Vraie fin de Rainbow Dash--"

# game/Scripts/dashcloud.rpy:398
translate French yummymuffin_d5421b1d:

    # "Rainbow Dash isn't there. It would be creepy to wait for her in her bed."
    "Rainbow Dash n'est pas là. Ce serait effrayant de l'attendre dans son lit."

# game/Scripts/dashcloud.rpy:399
translate French yummymuffin_00755e5a:

    # p "Rainbow Dash is not here and I have nothing to do there. Let's go somewhere else."
    p "Rainbow Dash n'est pas là et je n'ai rien à faire ici. Allons ailleurs."

# game/Scripts/dashcloud.rpy:403
translate French yummymuffin_c55c2acb:

    # "And so you went back to ponyville and its crossroad."
    "Et ainsi vous êtes retourné à Ponyville et son carrefour."

