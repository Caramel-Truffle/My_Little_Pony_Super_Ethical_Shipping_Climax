# TODO: Translation updated at 2025-11-26 12:29

# game/Scripts/script.rpy:516
translate Engrish start_497c87f4:

    # y "AAAAAAAAAAAH!! Giant-pointy-pony-head!!"
    y "AAAAAAAAAAAH!! Big sharp horse face!!"

# game/Scripts/script.rpy:517
translate Engrish start_70111e48:

    # c "I'm Colgate! Welcome to the realm between our worlds!"
    c "Identification Colgate! Greetings interdimensional realm!"

# game/Scripts/script.rpy:518
translate Engrish start_d68ab443:

    # y "Wait, like the Colgate in My Little Pony? Am I dreaming or something?"
    y "Pause, Colgate from My Little Pony? Subconscious state or alternative?"

# game/Scripts/script.rpy:519
translate Engrish start_0ea8f8d7:

    # c "Not at all! Trixie just left, I'm going to take care of you! You know, about the \"let's change you into a red pony and have some special fun with Ponyville inhabitants and stuff\"!"
    c "Negative entirely! Trixie departed, personal responsibility assumed! Comprehension, regarding \"transformation crimson equine, entertainment Ponyville residents etcetera\"!"

# game/Scripts/script.rpy:521
translate Engrish start_71a86863:

    # y "..."
    y "\[silence\]"

# game/Scripts/script.rpy:522
translate Engrish start_d8983fd6:

    # y "Wrong game."
    y "Bad game."

# game/Scripts/script.rpy:523
translate Engrish start_a6fc5652:

    # c "Hum. So, I will change you into one of the mane 6 instead! And then I will send you in Ponyville and and-"
    c "Contemplation. Therefore, transformation toward primary sextet alternative! Subsequently Ponyville transmission plus-"

# game/Scripts/script.rpy:524
translate Engrish start_a06862f5:

    # y "Please calm down and do your stuff. I will see what happen myself."
    y "Request tranquility, execute procedure. Observation personal."

# game/Scripts/script.rpy:528
translate Engrish character_selection_75deb98c:

    # c "Who do you want to be? All have white teeth!"
    c "Identity preference? Universal dental whiteness!"

# game/Scripts/script.rpy:561
translate Engrish name_choice_e69e42b8:

    # c "No."
    c "Negative."

# game/Scripts/script.rpy:566
translate Engrish end_name_choice_3929edae:

    # y "I want to be [playername!t]."
    y "Desire existence as [playername!t]."

# game/Scripts/script.rpy:567
translate Engrish end_name_choice_72736899:

    # c "So, you are [playername!t] now."
    c "Therefore, identity [playername!t] currently."

# game/Scripts/script.rpy:568
translate Engrish end_name_choice_43cc34db:

    # p "Really?"
    p "Truthfully?"

# game/Scripts/script.rpy:569
translate Engrish end_name_choice_a78e8022:

    # c "Yes! Just look at yourself in the mirror!"
    c "Affirmative! Reflective surface observation personal!"

# game/Scripts/script.rpy:570
translate Engrish end_name_choice_e2b1043c:

    # p "What mirror? It's too dark here!"
    p "Reflective surface location? Excessive darkness present!"

# game/Scripts/script.rpy:571
translate Engrish end_name_choice_1edebcfe:

    # c "Whoops, my bad. I will just send you to Ponyville then! See you soon!"
    c "Error acknowledged. Ponyville transmission executed! Imminent encounter!"

# game/Scripts/script.rpy:572
translate Engrish end_name_choice_02b95c26:

    # p "Wait, wha-"
    p "Pause, interrogative-"

# game/Scripts/script.rpy:576
translate Engrish end_name_choice_74eae990:

    # "Poof!"
    "Vanishment!"

# game/Scripts/script.rpy:577
translate Engrish end_name_choice_70b222ec:

    # p "Hey, who are you? And why is it so dark there?"
    p "Attention, identification request? Darkness rationale location?"

# game/Scripts/script.rpy:578
translate Engrish end_name_choice_70a7f161:

    # "I'm the narrator and you're in a transition between two scenes. Now, let's stop meta-talking."
    "I story voice and you in change between two scene. Now, stop meta-talk."

translate Engrish strings:

    # game/Scripts/script.rpy:298
    old "Twilight Sparkle"
    new "Sparkle Book Horse"

    # game/Scripts/script.rpy:299
    old "Pinkie Pie"
    new "Pink Party Horse"

    # game/Scripts/script.rpy:300
    old "Fluttershy"
    new "Yellow Shy Horse"

    # game/Scripts/script.rpy:301
    old "Rainbow Dash"
    new "Blue Fast Horse"

    # game/Scripts/script.rpy:302
    old "Applejack"
    new "Orange Farm Horse"

    # game/Scripts/script.rpy:303
    old "Rarity"
    new "White Gem Horse"

    # game/Scripts/script.rpy:304
    old "Big Macintosh"
    new "Big Red Brother"

    # game/Scripts/script.rpy:532
    old "Twilight"
    new "Book Horse"

    # game/Scripts/script.rpy:533
    old "Twi'"
    new "Twi'"

    # game/Scripts/script.rpy:537
    old "Pinkie"
    new "Pink Horse"

    # game/Scripts/script.rpy:538
    old "Pinkeh"
    new "Pinkeh"

    # game/Scripts/script.rpy:543
    old "AJ"
    new "AJ"

    # game/Scripts/script.rpy:547
    old "Rainbow"
    new "Fast Horse"

    # game/Scripts/script.rpy:548
    old "Dashie"
    new "Dashie"

    # game/Scripts/script.rpy:558
    old "'Shy"
    new "'Shy"

    # game/Scripts/script.rpy:560
    old "Wait!! I want to be my OC!! It's a super-awesome alicorn with neon colors and stuff!!!!"
    new "Wait!! I want be my OC!! It super-awesome alicorn with neon colors and stuff!!!!"

