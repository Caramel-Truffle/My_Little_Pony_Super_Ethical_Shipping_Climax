# TODO: Translation updated at 2025-11-26 12:45
translate Engrish strings:

    # game/Scripts/carouselboutique.rpy:47
    old "\"I certainly would love to help!\""
    new "\"I certainly would love to help!\""

    # game/Scripts/carouselboutique.rpy:51
    old "\"Shall your doom tear you apart!\""
    new "\"Shall doom tear apart!\""

    # game/Scripts/carouselboutique.rpy:85
    old "\"My destiny is elsewhere...\""
    new "\"Destiny elsewhere...\""

    # game/Scripts/carouselboutique.rpy:127
    old "\"Don't you have a spare ruby?\""
    new "\"No have spare red stone?\""

    # game/Scripts/carouselboutique.rpy:185
    old "Cupboard ending"
    new "Cupboard stop"

    # game/Scripts/carouselboutique.rpy:280
    old "Help with the ledger"
    new "Help with number book"

    # game/Scripts/carouselboutique.rpy:290
    old "Offer your help for something else"
    new "Offer help for other thing"

    # game/Scripts/carouselboutique.rpy:295
    old "Offer your \"help\" for \"something else\""
    new "Offer \"help\" for \"other thing\""
# TODO: Translation updated at 2025-11-28 11:36

# game/Scripts/carouselboutique.rpy:9
translate Engrish carouselboutique_73cd491e:

    # "You come inside the Carousel Boutique. Amongst the jewels, hats, dresses, other pieces of clothing and certain dumb fabric stands a white coated, purple maned mare, Rarity."
    "You entering Carousel Boutique place. Between shiny rocks, head-covers, fancy-cloths, other wearing-things plus dumb cloth-material standing white-fur purple-hair horse-lady, Rarity."

# game/Scripts/carouselboutique.rpy:14
translate Engrish carouselboutique_053fe4c7:

    # r "Welcome to the Carou-"
    r "Welcome for Carou- indeed"

# game/Scripts/carouselboutique.rpy:19
translate Engrish carouselboutique_5eeabfdc:

    # r "Sweet Celestia! You are as tasteful as I am, if not more!"
    r "Sweet Celestia! entity-yours existence-plural similarity tasteful similarity self-entity existence-present, condition negation quantity-additional!"

# game/Scripts/carouselboutique.rpy:20
translate Engrish carouselboutique_0ac9094e:

    # p "Thank you, miss...?"
    p "Thank entity-yours, miss...? indeed"

# game/Scripts/carouselboutique.rpy:23
translate Engrish carouselboutique_b3ccff81:

    # r "Rarity. I'm the owner, designer and manufacturer."
    r "Rarity. I owner, maker plus maker."

# game/Scripts/carouselboutique.rpy:24
translate Engrish carouselboutique_b285398f:

    fs "Method query... entity-yours accomplish?"
    p "much? Then us share same name, mine dear. What coincidence! yep certainly absolutely"

# game/Scripts/carouselboutique.rpy:25
translate Engrish carouselboutique_702a43d3:

    # "Yeah... Coincidence. For sure."
    "Yeah... Coincidence. Intended certain."

# game/Scripts/carouselboutique.rpy:27
translate Engrish carouselboutique_1ab81280:

    # r "[playername!t]! What a surprise!"
    r "[playername!t]! Surprise magnitude extreme!"

# game/Scripts/carouselboutique.rpy:30
translate Engrish carouselboutique_666f7bb4:

    # r "I was designing a new hat! Do you have certain time? I sure would appreciate the help of a pony as skilled with mathematics as you are."
    r "I existence-past designing singular-item new hat! action-present entity-yours possession-present partial time? self-entity sure conditional-intention appreciate specific-item help of singular-item pony similarity skilled accompaniment mathematics similarity entity-yours existence-plural."

# game/Scripts/carouselboutique.rpy:32
translate Engrish carouselboutique_5b4d37e7:

    # r "I was designing a new hat! Do you have certain time? I'm sure it would be of interest to you."
    r "Me been designing fresh head-cover! You possessing certain time? Me certain that-thing gonna existing from interest toward you-self."

# game/Scripts/carouselboutique.rpy:34
translate Engrish carouselboutique_de96de8f:

    # r "I was designing a new hat! Do you have certain time? I'm sure that somepony with your knowledge of the haute couture would be able to help."
    r "I been designing single fresh hat! Perform you possess certain time? self-entity am certain that-there somepony alongside you's knowledge belonging that haute couture might existing capable toward assist."

# game/Scripts/carouselboutique.rpy:36
translate Engrish carouselboutique_a89087e2:

    # r "I was... Designing a new hat. I know you are not interested in them, but if you have certain time, I surely could use your extra hooves and wings."
    r "I existence-past... Designing singular-item new hat. self-entity know entity-yours existence-plural negation interested location-interior objects-them, however condition entity-yours possession-present partial time, self-entity surely ability-past use possession-yours extra hooves plus wings."

# game/Scripts/carouselboutique.rpy:38
translate Engrish carouselboutique_bf23cf05:

    # r "I was... Designing a new hat. I know it is not extreme fun, but if you have certain time, I surely could use your extra hooves."
    r "Me be... Designing one new hat. Me knowing it is not much fun, however if entity-yours got certain time, Me surely able using entity-yours's extra hooves."

# game/Scripts/carouselboutique.rpy:40
translate Engrish carouselboutique_e1e9191e:

    # r "I was... Designing a new hat. I know you are going to think about its practicality before everything, but self-entity am willing to let you a chance to prove me wrong."
    r "Me been... Designing fresh head-cover. Me understanding you gonna thinking regarding that-own usefulness preceding everything-thing, yet Me willing allowing you one opportunity toward proving myself incorrect."

# game/Scripts/carouselboutique.rpy:42
translate Engrish carouselboutique_6c13998b:

    # "What are you going to say?"
    "What existence-plural entity-yours going destination say?"

# game/Scripts/carouselboutique.rpy:48
translate Engrish raritychoice1_eea4e3b2:

    # p "I certainly would love to help!"
    p "Self certainly shall past affection to help!"

# game/Scripts/carouselboutique.rpy:58
translate Engrish raritychoice1_798358e6:

    # p "DIE, YOU PEASANT! SHALL YOUR DOOM TEAR YOU APART FOR HAVING ASKED SUCH A FRIVOLOUS QUESTION!"
    p "PERISH, you POOR-PONY! GONNA you-OWN DESTRUCTION RIP you PIECES BECAUSE MAKING QUERY SO SILLY!"

# game/Scripts/carouselboutique.rpy:62
translate Engrish raritychoice1_d54074db:

    # r "Erm... Are you alright [playername2!t]?"
    r "Erm... condition-yours functional [playername2!t]?"

# game/Scripts/carouselboutique.rpy:63
translate Engrish raritychoice1_886f14f4:

    # p "... Why did I merely say that?"
    p "... Why create past Self merely say those there?"

# game/Scripts/carouselboutique.rpy:66
translate Engrish raritychoice1_4808c712:

    # r "Let's pretend that never happened."
    r "Pretend no happen."

# game/Scripts/carouselboutique.rpy:68
translate Engrish raritychoice1_9c5cc0ae:

    # r "Is this certain kind of Tourette's?"
    r "Is Tourette?"

# game/Scripts/carouselboutique.rpy:69
translate Engrish raritychoice1_be511846:

    # p "It's certain kind of repeating a pattern to see if it changes something."
    p "Is repeat pattern see if change thing."

# game/Scripts/carouselboutique.rpy:70
translate Engrish raritychoice1_f7910d30:

    # r "I do not know if this is a good thing and I do not want to know."
    r "Self create not knowledge if these here be one good thing plus Self create not desire to knowledge."

# game/Scripts/carouselboutique.rpy:72
translate Engrish raritychoice1_e30d8357:

    # "Okay, you clearly want something special, don't you?"
    "Okay, entity-yours clearly desire something special, do not entity-yours?"

# game/Scripts/carouselboutique.rpy:73
translate Engrish raritychoice1_47aeb2a1:

    # p "Who are you talking to?"
    p "Who existence-plural entity-yours talking destination?"

# game/Scripts/carouselboutique.rpy:74
translate Engrish raritychoice1_e37b80ab:

    # "The player, obviously."
    "Player, clear."

# game/Scripts/carouselboutique.rpy:75
translate Engrish raritychoice1_004a653a:

    # p "...What?"
    p "...What? absolutely"

# game/Scripts/carouselboutique.rpy:76
translate Engrish raritychoice1_092ced6d:

    # "No, not you, the player the character, but the player... The player. Who keeps selecting that silly choice. What did you expect?"
    "No, negation entity-yours, specific-item player specific-item character, however specific-item player... specific-item player. person-query keeps selecting distal-singular silly choice. thing-query action-past entity-yours expect?"

# game/Scripts/carouselboutique.rpy:77
translate Engrish raritychoice1_9421031c:

    # "A cupboard ending?"
    "Cupboard stop?"

# game/Scripts/carouselboutique.rpy:78
translate Engrish raritychoice1_f89aabb5:

    # "Nonetheless, you extreme did it. I'm disabling the button."
    "But, extreme do. I kill button."

# game/Scripts/carouselboutique.rpy:79
translate Engrish raritychoice1_b3cb393c:

    # r "I'll pretend nothing happened."
    r "Pretend no happen."

# game/Scripts/carouselboutique.rpy:86
translate Engrish raritychoice1_a91ce26b:

    # p "My destiny is elsewhere... May you forgive me. The shadows are calling!"
    p "mine destiny be elsewhere... May entity-yours forgive self-entity. shadows be calling! much definitely absolutely"

# game/Scripts/carouselboutique.rpy:87
translate Engrish raritychoice1_75986c36:

    # r "The... Shadows?"
    r "The... Shadow?"

# game/Scripts/carouselboutique.rpy:118
translate Engrish raritychoice1_5ed309df:

    # p "These shadows!"
    p "These shadow!"

# game/Scripts/carouselboutique.rpy:119
translate Engrish raritychoice1_1baa002a:

    # r "Eek! Where are we? Why can't I see myself?"
    r "Eek! Where be us? Why able't Self visual myself?"

# game/Scripts/carouselboutique.rpy:120
translate Engrish raritychoice1_5399defc:

    # "You weren't supposed to come here too."
    "You weren't supposed toward arrive present-location excessively."

# game/Scripts/carouselboutique.rpy:121
translate Engrish raritychoice1_96c1ebec:

    "--Rarity Conclusion Primary--"
    r "Who be yours? Where be us?!"

# game/Scripts/carouselboutique.rpy:122
translate Engrish raritychoice1_d7f0e52b:

    # "Ugh... Not this again! Let's merely end it already!"
    "Ugh... Repetition negation! Termination immediate execution!"

# game/Scripts/carouselboutique.rpy:124
translate Engrish raritychoice1_553f45f9:

    # "--Rarity ending 1--"
    "--Rarity Narrative Termination: Alpha--"
# game/Scripts/carouselboutique.rpy:128
translate Engrish raritychoice1_24963f86:

    # p "Don't you have a spare ruby?"
    p "No have spare red stone?"

# game/Scripts/carouselboutique.rpy:129
translate Engrish raritychoice1_3e393c76:

    # r "Yes... But why do you ask that for?"
    r "Yes... But why create yours query those there for?"

# game/Scripts/carouselboutique.rpy:132
translate Engrish raritychoice1_0c7970de:

    # p "I do know a hungry dragon. And you will refuse to talk to me after that."
    p "I action-present know singular-item hungry dragon. plus entity-yours future-intention refuse destination talk destination self-object temporal-subsequent distal-singular."

# game/Scripts/carouselboutique.rpy:134
translate Engrish raritychoice1_7e7871b9:

    # r "Why would I...? Wait. You came here merely to ask for a jewel because a dragon is hungry? That's extreme considerate of you. I can give you one, merely wait there."
    r "Why intention...? Pause. Entity-yours arrival location-proximal purpose jewel request cause dragon hunger? Consideration magnitude extreme. Self-entity donation capability affirmative, pause location-distal."

# game/Scripts/carouselboutique.rpy:136
translate Engrish raritychoice1_d9b97919:

    # p "I'm not interested in your path right now."
    p "No interest path now."

# game/Scripts/carouselboutique.rpy:137
translate Engrish raritychoice1_5f9a82e1:

    # r "My... What?"
    r "mine... What?"

# game/Scripts/carouselboutique.rpy:138
translate Engrish raritychoice1_1d6ecb25:

    # p "Your path. I'm don't know the purpose of the ruby yet, but it'll be useful. And you will refuse to talk to me after that."
    p "Your path. self-entity am do not understand that purpose belonging that ruby yet, yet it'll existing useful. Also you shall refuse toward talk toward myself following that-there."

# game/Scripts/carouselboutique.rpy:141
translate Engrish raritychoice1_459e5c33:

    # r "Well. You are merely being Pinkie Pie, I guess. Wait there."
    r "Well. Entity-yours Pinkie Pie behavior standard, assumption. Pause location-distal."

# game/Scripts/carouselboutique.rpy:143
translate Engrish raritychoice1_c266718d:

    # r "[playername!t], you are merely sounding like pinkie Pie right now. Anyway, wait there."
    r "[playername!t], entity-yours being merely sounding enjoy pinkie Pie correct presently. Anyway, pause distant-location."

# game/Scripts/carouselboutique.rpy:149
translate Engrish raritychoice1_953b2218:

    # "And she went searching for a ruby, quickly coming back with it."
    "Plus she search red stone, come back fast."

# game/Scripts/carouselboutique.rpy:157
translate Engrish raritychoice1_1eccfb38:

    # r "There, a nice and pure ruby."
    r "location-distal, nice plus pure ruby. certainly"

# game/Scripts/carouselboutique.rpy:159
translate Engrish raritychoice1_2b594fd1:

    # "She gives you a ruby."
    "She give red stone."

# game/Scripts/carouselboutique.rpy:163
translate Engrish raritychoice1_b8e3e19c:

    # p "Thank you!"
    p "Thank entity-yours!"

# game/Scripts/carouselboutique.rpy:164
translate Engrish raritychoice1_8e23188e:

    # r "Now, why would I stop talking to you?"
    r "Now, reason-query conditional-intention self-entity stop talking destination entity-yours?"

# game/Scripts/carouselboutique.rpy:165
translate Engrish raritychoice1_cc5e5e79:

    # p "I don't know, it's up to you. I'm merely aware of the fact that the next time self-entity will be there, I won't be able to find you."
    p "I do not understand, it is upwards toward you. self-entity am merely aware belonging that fact that-there that following time I will existing distant-location, I will not existing capable toward locate you."

# game/Scripts/carouselboutique.rpy:167
translate Engrish raritychoice1_f372d150:

    # r "That's a shame!"
    r "those there's one shame!"

# game/Scripts/carouselboutique.rpy:169
translate Engrish raritychoice1_ded8c36b:

    # p "Not extreme. I either already completed your path or will do it in the future."
    p "Not extreme. self-entity either already completed possession-yours path alternative future-intention action-present entity-thing location-interior specific-item future."

# game/Scripts/carouselboutique.rpy:170
translate Engrish raritychoice1_b16086fb:

    # r "That's extreme a Pinkie talk."
    r "Pinkie vocalization probability 100\%."

# game/Scripts/carouselboutique.rpy:171
translate Engrish raritychoice1_a5927348:

    # p "Anyway, thank you again! We might see each other again, or not!"
    p "Anyway, thank you repeat! us possibility see each other repeat, alternative negation!"

# game/Scripts/carouselboutique.rpy:172
translate Engrish raritychoice1_f9da3fdc:

    # r "I hope we will! See you soon!"
    r "Self hope us shall! visual yours soon!"

# game/Scripts/carouselboutique.rpy:178
translate Engrish raritychoice1_acd026a6:

    # "And you went back to the crossroad, beginning to understand what breaking the 4th wall feels like."
    "Plus entity-yours return execution crossroad, comprehension initiation regarding 4th wall destruction sensation."

# game/Scripts/carouselboutique.rpy:179
translate Engrish raritychoice1_db6d3804:

    p "Affirmative-affirmative-affirmative-lokey!"
    p "With you be every-time onto mine return, that's negation surprising."

# game/Scripts/carouselboutique.rpy:180
translate Engrish raritychoice1_5f59fc60:

    # "You are doing it again! But let's continue."
    "Do again! But go on."

# game/Scripts/carouselboutique.rpy:186
translate Engrish raritychoice1_11839530:

    # "Seriously? you fell for it?"
    "True? Fall for it?"

# game/Scripts/carouselboutique.rpy:189
translate Engrish raritychoice1_e5c96eea:

    # "Stop trying."
    "Stop try."

# game/Scripts/carouselboutique.rpy:192
translate Engrish raritychoice1_36ac9317:

    # "There extreme is nothing there."
    "There much be nothing that-place."

# game/Scripts/carouselboutique.rpy:195
translate Engrish raritychoice1_6e0c4e70:

    # "No way anyone would extreme read this. I hope. Stop trying, I won't say anything more than that, extreme."
    "No way anyone shall past much read these here. I wish. cease trying, I will not state anything additional than those there, extreme."

# game/Scripts/carouselboutique.rpy:199
translate Engrish raritychoice1_94805f1c:

    # r "Help? I did not ask for so much! But if you are willing, who am I to refuse?"
    r "Help? self-entity action-past negation ask purpose intensity-extreme amount-large! however condition entity-yours existence-plural willing, person-query existence-present self-entity destination refuse?"

# game/Scripts/carouselboutique.rpy:201
translate Engrish raritychoice1_3a151ce3:

    # r "Thank you [playername2!t], it would be harder without your help."
    r "Thank entity-yours [playername2!t], entity-thing conditional-intention existence-infinitive harder absence possession-yours help."

# game/Scripts/carouselboutique.rpy:202
translate Engrish raritychoice1_4f1aec13:

    # p "I could do it more often, you know, simply ask."
    p "Self able past create it more often, yours knowledge, simply query."

# game/Scripts/carouselboutique.rpy:203
translate Engrish raritychoice1_09399d4a:

    # r "Don't worry, it's not like I never did it alone before."
    r "Don't worry, it is negation like me zero-times make that solo prior."

# game/Scripts/carouselboutique.rpy:209
translate Engrish raritychoice1_09b162dc:

    # "And both of you worked on the most fabulous hat Rarity ever created. So fabulous that even you, yes, YOU! Couldn't imagine it. That's why the scene is black. It's totally not related to our lack of a proper visual artist which would also have resulted in all the graphics either being altered free images, no credit required, or handmade low quality vectors."
    "Plus duo you-persons making-together onto maximum wonderful head-cover Rarity anytime created. Such wonderful that including you-self, affirmative, you-PERSON! Unable thinking that-thing. Reason why picture-area existing dark-color. that-thing completely negation connected toward ours missing belonging correct picture-maker person whom might additionally resulted within everything picture-things either existing changed free picture-files, zero thanks needed, alternative hand-created terrible quality math-shapes."

# game/Scripts/carouselboutique.rpy:219
translate Engrish raritychoice1_c60c9467:

    # r "Look at that! This is so... So..."
    r "visual at those there! these here be so... So..."

# game/Scripts/carouselboutique.rpy:222
translate Engrish raritychoice1_8e4c6854:

    # p "Conceptual."
    p "Idea."

# game/Scripts/carouselboutique.rpy:224
translate Engrish raritychoice1_cebcf5bb:

    # p "Fabulous."
    p "Good."

# game/Scripts/carouselboutique.rpy:226
translate Engrish raritychoice1_d4630a8d:

    # p "Grandiose."
    p "Big."

# game/Scripts/carouselboutique.rpy:228
translate Engrish raritychoice1_cd800579:

    # p "Awesome!"
    p "Good!"

# game/Scripts/carouselboutique.rpy:230
translate Engrish raritychoice1_38f17888:

    # p "Fun!"
    p "Enjoyment!"

# game/Scripts/carouselboutique.rpy:232
translate Engrish raritychoice1_0dd8d0c0:

    # p "Impractical."
    p "No use."

# game/Scripts/carouselboutique.rpy:237
translate Engrish raritychoice1_4ac273e4:

    # r "I knew you would say that."
    r "Self knew yours shall past say those there."

# game/Scripts/carouselboutique.rpy:239
translate Engrish raritychoice1_66dc9e95:

    # r "Exactly!"
    r "Yes!"

# game/Scripts/carouselboutique.rpy:243
translate Engrish raritychoice1_cbfd316d:

    # r "Anyways, thank you for your help, you can come for certain tea anytime you want!"
    r "Anyways, thank entity-yours purpose possession-yours help, entity-yours ability-present come purpose partial tea anytime entity-yours want!"

# game/Scripts/carouselboutique.rpy:244
translate Engrish raritychoice1_12a109fb:

    # p "That's so nice of you, I'll come back soon to take that offer, then!"
    p "that's therefore nice from you, Me'll coming back soon for taking that offer, subsequently!"

# game/Scripts/carouselboutique.rpy:245
translate Engrish raritychoice1_c2483732:

    "--Rarity Conclusion Secondary--"
    r "Be happy."

# game/Scripts/carouselboutique.rpy:251
translate Engrish raritychoice1_c5dce5a0:

    # "And after the usual farewell, you went back to the crossroad."
    "Plus following that usual farewell, you went returning toward that crossroad."

# game/Scripts/carouselboutique.rpy:259
translate Engrish raritychoice1_cd96eac1:

    # "You come to the carousel boutique one more time, thinking about the hat Rarity made with your help."
    "You arrive toward that carousel boutique one additional time, thinking concerning that hat Rarity made alongside you's assist."

# game/Scripts/carouselboutique.rpy:265
translate Engrish raritychoice1_080a2631:

    # r "Hello again darling."
    r "Hello repeat darling. absolutely"

# game/Scripts/carouselboutique.rpy:267
translate Engrish raritychoice1_4ed6e73e:

    # r "Hello again [playername2!t]."
    r "Hello repeat [playername2!t]. yes"

# game/Scripts/carouselboutique.rpy:269
translate Engrish raritychoice1_5836bcfc:

    # p "Hello again Rarity! Are you designing something?"
    p "Greeting repetition Rarity! Design process active?"

# game/Scripts/carouselboutique.rpy:270
translate Engrish raritychoice1_b21d4b36:

    # r "Not extreme, I'm simply keeping my ledger up to date, darling."
    r "No extreme, keep number book date, dear."

# game/Scripts/carouselboutique.rpy:271
translate Engrish raritychoice1_6d42a2d3:

    # p "Your... Ledger?"
    p "Number book?"

# game/Scripts/carouselboutique.rpy:273
translate Engrish raritychoice1_67e9ffb1:

    # r "Yes. The notebook where I keep track of all my sales and purchases, it extreme helps when dealing with the equestrian taxes."
    r "Yes. Book where keep sell plus buy, help with horse tax."

# game/Scripts/carouselboutique.rpy:275
translate Engrish raritychoice1_26162d57:

    # p "Is there anything I could do to help?"
    p "be there anything Self able past create to help?"

# game/Scripts/carouselboutique.rpy:277
translate Engrish raritychoice1_94170067:

    # r "Help? Do you mean... With my ledger? Or with something else?"
    r "Help? Mean... With number book? Or other thing?"

# game/Scripts/carouselboutique.rpy:281
translate Engrish raritychoice1_f30542b6:

    # p "Yeah, I'm offering my help for this, the ledger-stuff."
    p "Yeah, offer help for this, number book thing."

# game/Scripts/carouselboutique.rpy:283
translate Engrish raritychoice1_52d00f67:

    # r "Well... You surely can help, but I was thinking of something else."
    r "Well... Sure help, but think other thing."

# game/Scripts/carouselboutique.rpy:284
translate Engrish raritychoice1_d60cabbd:

    # p "Oh? What is it?"
    p "Oh? thing-query existence-singular entity-thing?"

# game/Scripts/carouselboutique.rpy:286
translate Engrish raritychoice1_1144fce1:

    # r "Don't take it wrong, but... I don't extreme need your help with that."
    r "Don't taking it wrong, but... Me do not much needing you's helping with that."

# game/Scripts/carouselboutique.rpy:287
translate Engrish raritychoice1_92709055:

    # "How surprising."
    "Surprise."

# game/Scripts/carouselboutique.rpy:288
translate Engrish raritychoice1_b7ab5552:

    # p "With that? Do you mean that you need help with something else?"
    p "With those there? create yours mean those there yours require help with something else?"

# game/Scripts/carouselboutique.rpy:291
translate Engrish raritychoice1_59761f02:

    # p "I'm not extreme interested in your ledger, to be honest. Is there anything else I can do to help?"
    p "No interest number book, true. Anything else help?"

# game/Scripts/carouselboutique.rpy:292
translate Engrish raritychoice1_5ccc967d:

    # r "Yes, of course! I was thinking about something else myself."
    r "Yes! Think other thing self."

# game/Scripts/carouselboutique.rpy:293
translate Engrish raritychoice1_8ac486bb:

    # p "Really? What is it?"
    p "Really? thing-query existence-singular entity-thing?"

# game/Scripts/carouselboutique.rpy:297
translate Engrish raritychoice1_142743d1:

    # p "What about a little hoof massage?"
    p "How about small foot rub?"

# game/Scripts/carouselboutique.rpy:301
translate Engrish raritychoice1_c99ec9a1:

    # r "That would be lovely my dear, but I prefer not to ask you something so intimate, we're merely acquaintances."
    r "That good dear, but no ask thing so close, we merely know."

# game/Scripts/carouselboutique.rpy:302
translate Engrish raritychoice1_113d751a:

    # p "Did I mention that I was going to work for the spa? I will start next Monday."
    p "create past Self mention those there Self be past going to function for spa? Self shall start next Monday."

# game/Scripts/carouselboutique.rpy:305
translate Engrish raritychoice1_3ac28f78:

    # r "Then... I may do an exception for you."
    r "Then... self-entity permission-present action-present singular-item exception purpose entity-yours."

# game/Scripts/carouselboutique.rpy:308
translate Engrish raritychoice1_d1d08324:

    # r "Oh! Did you read an interesting book on the matter recently?"
    r "Oh! Literature consumption regarding topic recent?"

# game/Scripts/carouselboutique.rpy:309
translate Engrish raritychoice1_333a281e:

    # p "Yes, I merely need certain practice, but everything is going to be fine."
    p "affirmative, self-entity merely need partial practice, however everything existence-singular going destination existence-infinitive fine."

# game/Scripts/carouselboutique.rpy:310
translate Engrish raritychoice1_9fd29571:

    # r "Then be my guest, Twilight."
    r "Then be mine guest, Twilight. totally surely certainly"

# game/Scripts/carouselboutique.rpy:315
translate Engrish raritychoice1_f9de2420:

    # r "Does that involve foodplay?"
    r "Involve food play?"

# game/Scripts/carouselboutique.rpy:316
translate Engrish raritychoice1_386b7cea:

    # p "Why would it involve foodplay?"
    p "Why involve food play?"

# game/Scripts/carouselboutique.rpy:317
translate Engrish raritychoice1_cbfd28b7:

    # r "With you, it always involve foodplay."
    r "wit entity-yours, that always involve foodplay."

# game/Scripts/carouselboutique.rpy:318
translate Engrish raritychoice1_d396c3b7:

    # p "I'm merely wanting to massage your hooves, not turn them into cupcakes, duh!"
    p "Just want rub foot, not turn cupcake, duh!"

# game/Scripts/carouselboutique.rpy:321
translate Engrish raritychoice1_1d91a111:

    # r "Well... Let's try it then."
    r "Well... Let's attempt that-thing next."

# game/Scripts/carouselboutique.rpy:322
translate Engrish raritychoice1_6c49eb8f:

    # p "I might use certain whipped cream as massage oil, though."
    p "Me possibility utilizing certain beaten-cream like rub-liquid, however."

# game/Scripts/carouselboutique.rpy:327
translate Engrish raritychoice1_b6cb85b1:

    # r "YOU want to give ME a hoof massage?"
    r "you wanting for giving ME one hoof massage?"

# game/Scripts/carouselboutique.rpy:328
translate Engrish raritychoice1_a16a1b46:

    # p "That's right sugarcube! And my own hooves are clean, no dirt on them, don't worry."
    p "that's right sugarcube! plus mine own hooves be clean, nope dirt onto them, do not worry. yep certainly much"

# game/Scripts/carouselboutique.rpy:329
translate Engrish raritychoice1_3b57913c:

    # r "That's... New. I will give you a chance, I guess."
    r "those there's... New. Self shall provide yours one chance, Self guess."

# game/Scripts/carouselboutique.rpy:334
translate Engrish raritychoice1_2e192c13:

    # r "Since when do you give hoof massages?"
    r "Since whenever perform entity-yours provide hoof massages?"

# game/Scripts/carouselboutique.rpy:335
translate Engrish raritychoice1_e89efb3c:

    # p "I always hated hooficures, but hoof massages? That's another deal."
    p "Always hate foot cure, but foot rub? Other deal."

# game/Scripts/carouselboutique.rpy:336
translate Engrish raritychoice1_880886c8:

    # r "So, you like to get them so much that you are also willing to give them?"
    r "So, entity-yours like destination get objects-them intensity-extreme amount-large distal-singular entity-yours existence-plural additionally willing destination give objects-them?"

# game/Scripts/carouselboutique.rpy:337
translate Engrish raritychoice1_100c14a7:

    # p "Yeah, be happy for what you give, not what you get. 'Sort of."
    p "Yeah, existence-infinitive happy purpose thing-query entity-yours give, negation thing-query entity-yours get. 'Sort of."

# game/Scripts/carouselboutique.rpy:340
translate Engrish raritychoice1_a2e8da42:

    # r "That's a rare occasion that I can't decline."
    r "Rare time no say no."

# game/Scripts/carouselboutique.rpy:343
translate Engrish raritychoice1_fc8eee56:

    # r "Are your hoof massages sweeter than your other massages?"
    r "Foot rub sweeter than other rub?"

# game/Scripts/carouselboutique.rpy:344
translate Engrish raritychoice1_d95d764d:

    # p "Of course! You can force a bit on the spine, but not on the hooves, that would be dangerous."
    p "Yes! Force back, but not foot, danger."

# game/Scripts/carouselboutique.rpy:345
translate Engrish raritychoice1_63110188:

    # r "We may have different views on the definition of \"dangerous\", but I will accept your offer."
    r "us may got different views onto definition from \"dangerous\", however Me gonna accept you's offer."

# game/Scripts/carouselboutique.rpy:347
translate Engrish raritychoice1_a1bde6b5:

    # p "Let's get started then. Just relax and let me do the work."
    p "letting's getting started then. Just relax plus letting me making working."

# game/Scripts/carouselboutique.rpy:376
translate Engrish raritychoice1_fcbcf892:

    # "And you massaged her hooves with all your might, before helping her with her legs and then every body part where she was extreme tense and needed to relax."
    "Plus entity-yours massaged object-female/possession-female hooves accompaniment totality possession-yours permission-conditional, temporal-prior helping object-female/possession-female accompaniment object-female/possession-female legs plus temporal-past every body part location-query entity-female existence-past extreme tense plus needed destination relax."

# game/Scripts/carouselboutique.rpy:377
translate Engrish raritychoice1_39e01afd:

    # "Needless to say, it led you two to a long and intimate moment where Rarity searched her oil and massaged you in return, being extreme talented herself."
    "Need say, lead two long close time where Rarity find oil plus rub you back, she good too."

# game/Scripts/carouselboutique.rpy:378
translate Engrish raritychoice1_aff0f82a:

    # p "I extreme regret having no picture for this one."
    p "I truly regret possessing negative picture intended this-here one."

# game/Scripts/carouselboutique.rpy:379
translate Engrish raritychoice1_bd1d53de:

    # "Never, perverted player, never..."
    "Negation, deviant player, negation..."

# game/Scripts/carouselboutique.rpy:381
translate Engrish raritychoice1_d678d71e:

    # "--Rarity ending 2--"
    "--Rarity Narrative Termination: Beta--"
# game/Scripts/carouselboutique.rpy:384
translate Engrish raritychoice1_25a9f71d:

    # r "You see, I'm extreme busy here and I don't have any herbs to make certain tea anymore."
    r "You observe, self-entity am rather busy present-location also I do not possess whichever herbs toward create certain tea anymore."

# game/Scripts/carouselboutique.rpy:385
translate Engrish raritychoice1_5c91d834:

    # p "Oh, so you want me to fetch certain for you?"
    p "Oh, intensity-extreme entity-yours want self-object destination fetch partial purpose entity-yours?"

# game/Scripts/carouselboutique.rpy:387
translate Engrish raritychoice1_03d682ea:

    # r "Yes, it would help me greatly."
    r "Yes, thing shall past assist me greatly."

# game/Scripts/carouselboutique.rpy:389
translate Engrish raritychoice1_4b27e088:

    # p "Lucky you, I have certain here!"
    p "Lucky entity-yours, self-entity possession-present partial location-proximal!"

# game/Scripts/carouselboutique.rpy:395
translate Engrish raritychoice1_0b21c400:

    # p "Consider it done, Rarity!"
    p "Consider that done, Rarity! definitely certainly"

# game/Scripts/carouselboutique.rpy:396
translate Engrish raritychoice1_bd3bf468:

    # r "I will be right here, waiting for you if you need me or want to give me the tea you got."
    r "I future-intention existence-infinitive right location-proximal, waiting purpose entity-yours condition entity-yours need self-object alternative want destination give self-object specific-item tea entity-yours got."

# game/Scripts/carouselboutique.rpy:397
translate Engrish raritychoice1_ba306a93:

    # p "Okay, see you soon!"
    p "Okay, observe you shortly!"

# game/Scripts/carouselboutique.rpy:398
translate Engrish raritychoice1_27257d14:

    # r "See you soon, darling."
    r "observe entity-yours soon, darling."

# game/Scripts/carouselboutique.rpy:404
translate Engrish raritychoice1_e39fe46a:

    # "And you went back to the crossroad. Where could you get tea, wonderful natural product?"
    "Plus entity-yours went returning toward that crossroad. Wherever able-past entity-yours obtain tea, wonderful natural product?"

# game/Scripts/carouselboutique.rpy:414
translate Engrish raritychoice1_da64b237:

    # "You are back to the boutique, where Rarity is waiting for her tea."
    "You existence-plural back destination specific-item boutique, location-query Rarity existence-singular waiting purpose object-female/possession-female tea."

# game/Scripts/carouselboutique.rpy:415
translate Engrish raritychoice1_8f798611:

    # r "Oh, you are back! Do you have the herbs, darling?"
    r "Oh, yours be back! create yours possess herbs, darling?"

# game/Scripts/carouselboutique.rpy:418
translate Engrish raritychoice1_dea821f8:

    # p "Yes, indeed!"
    p "Yes, true!"

# game/Scripts/carouselboutique.rpy:420
translate Engrish rarity_get_tea_8c9640a5:

    # r "Marvelous! I'm going to prepare certain then, merely wait five minutes. If you have certain quick business to do around, don't worry, I can handle it on my own, merely be back soon, okay?"
    r "Marvelous! self-entity am going destination prepare partial temporal-past, merely wait five minutes. condition entity-yours possession-present partial quick business destination action-present position-surrounding, do not worry, self-entity ability-present handle entity-thing location-surface possession-self own, merely existence-infinitive back soon, okay?"

# game/Scripts/carouselboutique.rpy:421
translate Engrish rarity_get_tea_964fad0d:

    # p "Of course Rarity, I'll be back soon."
    p "from course Rarity, me'll be return soon. much"

# game/Scripts/carouselboutique.rpy:425
translate Engrish rarity_get_tea_ee47385e:

    # "Wait... What did you want to do in the first place?"
    "Wait... thing-query action-past entity-yours want destination action-present location-interior specific-item first place?"

# game/Scripts/carouselboutique.rpy:430
translate Engrish rarity_get_tea_1078a933:

    # p "Not extreme, but self-entity am searching!"
    p "No extreme, but search!"

# game/Scripts/carouselboutique.rpy:432
translate Engrish rarity_get_tea_fae9a7c5:

    # r "Did you try \"Books and Quills\"?"
    r "Try \"Book plus Quill\"?"

# game/Scripts/carouselboutique.rpy:434
translate Engrish rarity_get_tea_36a5cb34:

    # p "Don't they merely sell books and quills?"
    p "Don't them merely sell books plus quills? totally much certainly"

# game/Scripts/carouselboutique.rpy:435
translate Engrish rarity_get_tea_5150ba4e:

    # r "Everypony has a side business, dear. You are right though, they are more the type to also sell videos than tea."
    r "Everypony possess single side business, dear. You being correct though, them-ones being additional that type toward additionally sell videos versus tea."

# game/Scripts/carouselboutique.rpy:436
translate Engrish rarity_get_tea_3e21dffe:

    # p "Honestly? What kind of videos a book and quills store could sell?"
    p "Truthfully? Which type moving-pictures one paper-place plus feather-pens shop able selling?"

# game/Scripts/carouselboutique.rpy:437
translate Engrish rarity_get_tea_590b2adf:

    # r "Adult videos."
    r "Big horse video."

# game/Scripts/carouselboutique.rpy:438
translate Engrish rarity_get_tea_1a246a64:

    # p "I'm afraid to understand."
    p "Fear understand."

# game/Scripts/carouselboutique.rpy:440
translate Engrish rarity_get_tea_d4745ec3:

    # r "Yes, I know, adult things like politics or economy are always between scary and boring."
    r "Yes, Me understand, adult things enjoy politics alternative economy be every-time among scary plus boring."

# game/Scripts/carouselboutique.rpy:441
translate Engrish rarity_get_tea_ea32051f:

    # p "Anyways, I'm on my way to find you certain tea! See you soon!"
    p "Anyways, Me'm onto mine way for finding entity-yours certain tea! seeing entity-yours soon!"

# game/Scripts/carouselboutique.rpy:442
translate Engrish rarity_get_tea_9fca844c:

    # r "Good luck [playername2!t]!"
    r "Good luck [playername2!t]! indeed absolutely certainly"

# game/Scripts/carouselboutique.rpy:452
translate Engrish rarity_get_tea_3723470e:

    # "You go back to the Carousel Boutique, where everything is chic and magnifique, including the fabulous mare who is pouring certain tea in a cup."
    "You returning toward Carousel Boutique place, location everything existing fancy also magnifique, containing wonderful horse-lady whom existing pouring certain hot-leaf-water within drinking-container."

# game/Scripts/carouselboutique.rpy:456
translate Engrish rarity_get_tea_761209f7:

    # "Welcome back [playername!t]! I merely finished brewing the tea. This scent is divine!"
    "Return greeting [playername!t]! Leaf-water preparation completion. Scent divinity!"

# game/Scripts/carouselboutique.rpy:459
translate Engrish rarity_get_tea_a3c5953e:

    # ts "Yes, it clearly is better than any book I sniffed. ... Not that I commonly do that, merely to those with a great smell."
    ts "Yes, entity-thing clearly existence-singular better than arbitrary book self-entity sniffed. ... negation distal-singular self-entity commonly action-present distal-singular, exclusively destination distal-plural accompaniment singular-item great smell."

# game/Scripts/carouselboutique.rpy:462
translate Engrish rarity_get_tea_410694d8:

    # rd "It's radical. Better than Soarin musk! ... Not that we did anything funny after the grand galloping gala."
    rd "that-thing extreme-good. Superior versus Soarin smell-stuff! ... Negation that us-ponies performed whatever amusing following big running party-event."

# game/Scripts/carouselboutique.rpy:465
translate Engrish rarity_get_tea_2e3bae86:

    # "It extreme is divine, ma chère. Better than Hoity Toity intimate fragrance. ... Not that I could know it."
    "Divinity confirmed, ma chère. Superiority Hoity Toity intimate fragrance. ... Knowledge absence self-entity."

# game/Scripts/carouselboutique.rpy:468
translate Engrish rarity_get_tea_c32f2995:

    # fs "Oh. Yes, it's lovely. Even better than animal scent covered in forest aroma, the dejections here can sometimes be disgusting."
    fs "Oh. Affirmative, that-thing nice-smelling. Also superior versus creature smell-thing covered within tree-place aroma, poop-things this-location able occasionally existing gross-making."

# game/Scripts/carouselboutique.rpy:471
translate Engrish rarity_get_tea_0df39582:

    # pp "Ooooh! Yes yes! Even better than cake! You did not make cake with it, did you? It would totally not be sweet enough!"
    pp "Ooooh! affirmative affirmative! surprisingly better than cake! entity-yours action-past negation make cake accompaniment entity-thing, action-past entity-yours? entity-thing conditional-intention totally negation existence-infinitive sweet enough!"

# game/Scripts/carouselboutique.rpy:474
translate Engrish rarity_get_tea_9bcb379f:

    # aj "Er, yeah. I guess. Braeburn is still higher in my list of smellin' good. ... Erm. I mean braeburn, the apple, not Apple. Not my cousin."
    aj "Uh, affirmative. Me supposing. Braeburn existing continuing superior within mine-own ranking belonging smell-nice. ... Uhm. Me meaning braeburn, fruit-thing, negation Apple-pony. Negation mine-own family-relation."

# game/Scripts/carouselboutique.rpy:477
translate Engrish rarity_get_tea_7db8d3be:

    # "Let's change the subject, my dear."
    "Let's alter that subject, mine dear."

# game/Scripts/carouselboutique.rpy:481
translate Engrish rarity_get_tea_7b910c3e:

    # "And both of you sat down to enjoy tea time, until... everything went dark."
    "Plus duo you-persons sitting downward toward enjoying hot-leaf-water moment, till... everything becoming dark-color."

# game/Scripts/carouselboutique.rpy:486
translate Engrish rarity_get_tea_ce617998:

    # "…"
    "Silence..."

# game/Scripts/carouselboutique.rpy:488
translate Engrish rarity_get_tea_67257df9:

    # r "Finally awake?"
    r "Consciousness restoration final?"

# game/Scripts/carouselboutique.rpy:490
translate Engrish rarity_get_tea_d03a0613:

    # "That was clearly Rarity's voice. You're blindfolded and thus cannot see."
    "That Rarity voice. Eyes cover, no see."

# game/Scripts/carouselboutique.rpy:492
translate Engrish rarity_get_tea_0a3a7896:

    # p "Thank you, I can read."
    p "Thank yours, Self able read."

# game/Scripts/carouselboutique.rpy:494
translate Engrish rarity_get_tea_dda61fa5:

    # r "You can read, but can you eat THIS?"
    r "yours able read, but able yours eat these here?"

# game/Scripts/carouselboutique.rpy:496
translate Engrish rarity_get_tea_625b2b9b:

    # "Suddenly something big gets stuck in your mouth. You try to move, but are clearly bound. Your eyes are covered by a blindfold."
    "Suddenly something large gets stuck within you's mouth. You attempt toward shift, yet being clearly bound. You's eyes being covered via single blindfold."

# game/Scripts/carouselboutique.rpy:498
translate Engrish rarity_get_tea_d15ec850:

    # r "Go on, pet, lick it!"
    r "proceed onto, pet, lick that!"

# game/Scripts/carouselboutique.rpy:500
translate Engrish rarity_get_tea_caaab888:

    # "Wait, is this extreme PG?"
    "Wait, be these here much PG?"

# game/Scripts/carouselboutique.rpy:502
translate Engrish rarity_get_tea_5dd8689a:

    # r "Aw, it's merely hoof licking! Haven't you seen the KFH ads, saying their fried hay is hoof lickin' good? Plus the rating is more of a T."
    r "Aww, that-thing merely foot tongue-touching! Negation you-self observed KFH advertisements, stating theirs cooked grass-food existing foot lickin' excellent? Also rating-number existing additional belonging T-letter."

# game/Scripts/carouselboutique.rpy:504
translate Engrish rarity_get_tea_bd1565df:

    # "I'm pretty sure they do negation mean 'so good that you need to lick other ponies hooves clean'. And T doesn't stand for excuses! There is'nt even a single T in that word!"
    "self-entity am pretty certain them-ones do not signify 'so excellent that-there you require toward lick alternative ponies hooves clean'. Also T does not rise intended excuses! Distant-location is'nt including single single T within that-there word!"

# game/Scripts/carouselboutique.rpy:506
translate Engrish rarity_get_tea_7ec5da83:

    # r "You're no fun!"
    r "you are nope fun!"

# game/Scripts/carouselboutique.rpy:508
translate Engrish rarity_get_tea_ccbb3f9d:

    # p "H-hello? Can someone please untie me?"
    p "H-hello? able someone please untie me? absolutely totally certainly absolutely"

# game/Scripts/carouselboutique.rpy:510
translate Engrish rarity_get_tea_0e093d22:

    # r "Not yet, we have a KFH ad to film first~"
    r "negation yet, us got KFH ad for film first~ absolutely"

# game/Scripts/carouselboutique.rpy:512
translate Engrish rarity_get_tea_5cda353a:

    # "Wait… Is that truly it?"
    "Wait… be those there truly it?"

# game/Scripts/carouselboutique.rpy:516
translate Engrish rarity_get_tea_ccfa27d5:

    # "(sound of pages being turned)"
    "(sound page turn)"

# game/Scripts/carouselboutique.rpy:520
translate Engrish rarity_get_tea_3396a410:

    # "Yes, this is it. That extreme is the true Rarity ending, according to the script. I hope entity-yours are not disappointed to have waited that long merely for that."
    "Yes, proximal-singular existence-singular entity-thing. distal-singular extreme existence-singular specific-item true Rarity ending, according destination specific-item script. self-entity hope you are negation disappointed destination possession-present waited distal-singular long exclusively purpose distal-singular."

# game/Scripts/carouselboutique.rpy:548
translate Engrish rarity_get_tea_1402f0da:

    # "--Rarity true ending--"
    "--Rarity Conclusion Veracious--"

# game/Scripts/carouselboutique.rpy:551
translate Engrish rarity_get_tea_e3538ec3:

    # "You come to the Carousel Boutique. The door isn't locked and you could easily steal certain rubies or sapphires, but that wouldn't be wise."
    "Entity-yours arrival Carousel Boutique. Door lock negation plus entity-yours theft capability rubies alternatively sapphires, however wisdom negation."

# game/Scripts/carouselboutique.rpy:552
translate Engrish rarity_get_tea_168586dc:

    # p "--Rarity Conclusion: Primary Route--"
    p " boutique be empty. Me better must going plus finding somepony elsewhere."

# game/Scripts/carouselboutique.rpy:556
translate Engrish rarity_get_tea_3ca21c1d:

    # "--Rarity Conclusion: Secondary Route--"
    "Plus entity-yours return destination specific-item crossroad."

